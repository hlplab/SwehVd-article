...
Native talkers of Stockholm Swedish were recruited through word-of-mouth, flyers at Stockholm University Campus (see example flyer in SI \@ref(sec:recruitment)), and online channels (accindi.se). Participants were selected based on the following criteria: L1 talkers of Swedish, born and raised in the bigger Stockholm area or its surroundings, 20-40 years old (mean age=`r mean(d.SwehVd$Age) %>% round(digits = 0)`; SD=`r sd(d.SwehVd$Age) %>% round(digits = 2)`). Four of the participants were bilingual from birth, the L1s of each talker are provided in the database. All participants were reimbursed with a voucher to the value of SEK 100 after completing the recordings.
...
Similary, we largely limited the comparison of normalization accounts to a single level of normalization, or combinations of perceptual transformations and a single type of normalization. Previous findings however suggest that both intrinsic and extrinsic normalization seems to play a role [@nearey1989]. Evaluating one type of account at a time fails to address the call made by Nearey in 1989: "We clearly need models that are sensitive to both intrinsic and extrinsic effects of speaker variation.". Some proposals, however, can be understood as allowing multiple levels of normalization prior to the mapping from (normalized) cues to linguistic categories. For example, one proposal that dates back to at least Nearey [-@neareyXXX] and has recently been worked out and tested in more detail [@barreda2020a; @barreda2021; @nearey2018] holds that one of the earliest normalization is essentially specialized on separating vocal tract size from the signal that is used for vowel categorization. These accounts allow for the possibility that subsequent mechanisms involve more complex normalizations---a possibility not considered here.  
