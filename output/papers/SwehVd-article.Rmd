...
# To Do
  ...
  * FIGs
    ...
    * adjust fig sizes by chunk arguments in the following plots: 10, 11 (also need smaller scaling), 13, 14.
...
& & \cellcolor[HTML]{DDADAF}{{\multirow[c]{-3}{*}{\rotatebox{90}{scaling}}}} & \cellcolor[HTML]{DDADAF}{(z-score)} & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{} \\
...
```{r swe-vowels-all-cues, fig.width=base.width*4, fig.height=base.height*4+.5, out.width = '100%', fig.cap="(ref:swe-vowels-all-cues)"}
p.matrix.cues <- 
  d.SwehVd %>% 
  filter(Transcribed_vowel == "targeted", Word != "hädd") %>%
...
p.separability.F3_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "F3")) + facet_grid(~ factor(Quantity, levels = c("long", "short", "all"))) + theme(plot.margin = unit(c(0, 0, -1.5, 0), "cm"))
p.separability.F2_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "F2")) + facet_grid(~ factor(Quantity, levels = c("long", "short", "all"))) + theme(plot.margin = unit(c(0, 0, -1.5, 0), "cm"))
p.separability.F1_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "F1")) + facet_grid(~ factor(Quantity, levels = c("long", "short", "all"))) + theme(plot.margin = unit(c(0, 0, -1.5, 0), "cm"))
p.separability.F0_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "F0")) + facet_grid(~ factor(Quantity, levels = c("long", "short", "all"))) + theme(plot.margin = unit(c(0, 0, -1.5, 0), "cm"))
...
```{r F3-densities, fig.width= base.width * 3.5, fig.height = base.height * 2.5, out.width='100%', fig.align='center', fig.cap="(ref:F3-densities)"}
# Generate list of manualy y-scale limits, in order to have category labels fit in at density tops
scales <- list(
...
plot_io_results <- function(data) {
  data %>%
    ...
    scale_colour_manual(
      ...
      values = colors.all.procedures) +
    guides(color = "none") +
    theme(axis.text.x = ggtext::element_markdown(angle = 45, vjust = 1, hjust = 1, colour = colors.all.procedures, size=8),
        axis.text.y = element_text(size=8, hjust=1, vjust=.5),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        strip.text = element_text(size=8, color = "white"),
...
plot_io_results_per_vowel <- function(data) {
  data %>%
    ...
    scale_colour_manual(
      ...
      values = colors.all.procedures) +
    guides(color = "none") +
    theme(axis.text.x = ggtext::element_markdown(angle = 90, vjust = 1, hjust = 1, colour = colors.all.procedures, size=8),
        axis.text.y = element_text(size=8, hjust=1, vjust=.5),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        strip.text = element_text(size=8, color = "white"),
