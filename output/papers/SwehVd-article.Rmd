...
d.max.acc.toINTEND %>% filter(IO.Normalization.Type %in% c("CCuRE_Hz", "CCuRE_Mel", "CCuRE_Bark", "CCuRE_ERB", "CCuRE_semitones", "Nearey1_log", "Nearey2_log", "Lobanov_Hz")) %>% group_by(Cue_space, IO.Quantity) %>% summarise(max = max(mean)) %>% ungroup() %>% summarise(min(max)) * 100
d.max.acc.toINTEND %>% filter(IO.Normalization.Type != "r_Hz") %>% group_by(Cue_space, IO.Quantity) %>% summarise(max = max(mean)) %>% ungroup() %>% summarise(max(max)) * 100
```
