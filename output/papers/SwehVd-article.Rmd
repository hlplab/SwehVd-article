...
plot_io_results_per_vowel <- function(data) {
  data %>%
    ...
    scale_colour_manual(
      ...
      values = colors.all.procedures) +
    guides(color = "none") +
    theme(axis.text.x = ggtext::element_markdown(angle = 90, vjust = 1, hjust = 1, colour = colors.all.procedures),
        axis.title.x = element_blank())
}

# p.accuracy.io.per.vowel.F1F2 <- plot_io_results_per_vowel(
#   m.io.SwehVd %>%
#     filter(IO.Quantity == "long")) + facet_grid(Cue_space ~ Intended.Vowel)
