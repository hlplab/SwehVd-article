...
# To Do
  ...
  * FIGs
    * fix fig sizes
    * write function for large_plot_theme
    * clean up the diphthong plot + add text
  * Have all tables be the same size
  * Adjust font size of plot labels, legends and titles (should be slightly smaller than caption label).
  * REFs
...
```{r illustrate-normalization, fig.width=base.width*3+.5, fig.height=base.height*3, out.width='100%', fig.align='center', fig.cap="(ref:illustrate-normalization)"}
cowplot::plot_grid(
  legend,
  cowplot::plot_grid(
    cowplot::plot_grid(p.Eng.LiXie.raw, p.Eng.LiXie.Lobanov, align = "h", labels = c("A", "B"), ncol = 2, hjust = -6, vjust = 1.7),
    cowplot::plot_grid(
      p.Eng.LiXie.all.raw + theme(legend.position = "none"), 
      p.Eng.LiXie.all.Lob, 
      NULL,
      align = "hv", labels = c("C", "D", ""), ncol = 1, rel_heights = c(1, 1, .35), hjust = -6, vjust = 1.9), 
    align = "hv", axis = "t", rel_widths = c(2/3, 1/3)), 
...
\multirow[c]{-4}{*}{\rotatebox{90}{trans-}} & & \cellcolor[HTML]{C9C0BB}{} & \cellcolor[HTML]{C9C0BB}{---} & \cellcolor[HTML]{C9C0BB}{Semitones conversion} & \cellcolor[HTML]{C9C0BB}{Fant et al. (2002)} & \cellcolor[HTML]{C9C0BB}{$F_n^{ST} = 12 \times \frac{ln(\frac{F_n}{100})}{ln}$} \\
& \multirow[c]{-6}{*}{\rotatebox{90}{formation}} & \cellcolor[HTML]{C9C0BB}{} & \cellcolor[HTML]{C9C0BB}{} & \cellcolor[HTML]{C9C0BB}{} & \cellcolor[HTML]{C9C0BB}{} & \cellcolor[HTML]{C9C0BB}{} \\

\hline

& & \cellcolor[HTML]{E6BE8A}{} & \cellcolor[HTML]{E6BE8A}{Syrdal \& Gopal's Bark-distance model} & \cellcolor[HTML]{E6BE8A}{Bark} & \cellcolor[HTML]{E6BE8A}{Syrdal \& Gopal (1986)} & \cellcolor[HTML]{E6BE8A}{$F1^{SyrdalGopal} = F1^{Bark} - F0^{Bark}$} \\
& & \cellcolor[HTML]{E6BE8A}{} & \cellcolor[HTML]{E6BE8A}{} & \cellcolor[HTML]{E6BE8A}{} & \cellcolor[HTML]{E6BE8A}{} & \cellcolor[HTML]{E6BE8A}{$F2^{SyrdalGopal} = F2^{Bark} - F1^{Bark}$} \\
...
& & \cellcolor[HTML]{ABCDEF}{{\multirow[c]{-11}{*}{\rotatebox{90}{centering}}}} & \cellcolor[HTML]{ABCDEF}{log-mean)} & \cellcolor[HTML]{ABCDEF}{} & \cellcolor[HTML]{ABCDEF}{} & \cellcolor[HTML]{ABCDEF}{} \\ 
...
& & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{Lobanov} & \cellcolor[HTML]{DDADAF}{Hz} & \cellcolor[HTML]{DDADAF}{Lobanov (1971)} & \cellcolor[HTML]{DDADAF}{$F^{Lobanov}_n = \frac{F_n - mean(F_n)}{sd(F_n)}$} \\
& & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{(z-score)} & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{} \\
& & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{} \\
\multirow[c]{-18}{*}{\rotatebox{90}{extrinsic}} & & \cellcolor[HTML]{DDADAF}{\multirow[c]{-7}{*}{\rotatebox{90}{scaling}}} & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{} & \cellcolor[HTML]{DDADAF}{} \\
...
Several of the long vowels are thought to be produced with some diphthongisation in Central Swedish. According to @elert2000, diphthongisation should result in formant frequency movements towards [`r linguisticsdown::cond_cmpl("ə")`]. The results from previous studies are somewhat inconclusive as to how widespread this diphthongisation is, and what direction it takes. @eklund1997's results indicated no diphthongisation of the low vowels [`r linguisticsdown::cond_cmpl("ɑː")`] and [`r linguisticsdown::cond_cmpl("æː")`], substantial diphthongisation of the mid, mid-high, vowels [`r linguisticsdown::cond_cmpl("eː")`], [`r linguisticsdown::cond_cmpl("øː")`] and [`r linguisticsdown::cond_cmpl("oː")`] but not towards [`r linguisticsdown::cond_cmpl("ə")`] but rather towards a more open quality. For the high vowels, @eklund1997 reported formant movements similar to @elert2000, but not as substantial. Future work could employ the SwehVd database to further address this question (some initial visualizations are provided in Section \@ref(sec:cue-extraction)). Of practical consequences for the current purpose is that diphthongisation---if present---could affect segmentation and formant measurements. We address this concern as part of Study 1.
...
```{r spectrogram, echo=FALSE, fig.cap="Example of Praat textgrid with annotated segment boundaries and measurement points for the automatic extraction of F1-F3 formant frequencies.", out.width = '40%', fig.align='center'}
...
Figure \@ref(fig:diphthongs) shows the trajectory of the average F1-F2 values across the five time-points for each of the vowels in the SwehVd database. It is clear from this plot that there is some movement across time-points, in particular for some of the long vowels. The vowels that exhibit most movement have been discussed in previous studies on diphthongisation, e.g., [`r linguisticsdown::cond_cmpl("eː")`] and  [`r linguisticsdown::cond_cmpl("oː")`] [e.g., @eklund1997; @elert2000]. The high front vowels (`[r linguisticsdown::cond_cmpl("[iː]")]`, [`r linguisticsdown::cond_cmpl("[yː]")`]) does not display any movement, the remaining vowels display some. The movement is primarily along F1, but also F2. There is also movement in some of the short vowels, which could also indicate co-articulation with the subsequent /d/.<!-- AP: check and cite papers here. e.g. Sussman et al., 1997, Parker, 1974; what do you expect from a final /d/? Also, check what context Eklund and Elert are using--> Across vowels, the movement is largest between the last two steps, at 65% and 80% of the vowel. We decided to exclude vowels where there was a significant systematic upward movement along F1. We did so by running separate mixed effect regressions for each vowel, with F1 as dependent variable and time-point as predictor, and the interaction of time-point and Talker as random effects.

(ref:diphthongs) The trajectory of all vowels across the five time points, along F1-F2. Each number corresponds to one of the time-points.
...
p.diphthongs.average <- 
  d.SwehVd %>%
    mutate(Location = as.character(Location)) %>%
    group_by(Talker, category, Location) %>%
    summarise(across(c(F1, F2), ~geometric.mean(.x))) %>%
...
# m <- lme4::lmer(F1~ Location + (1 + Location | Talker), data = d.SwehVd %>% filter(Quantity == "long", category == "[ɑː]"))
# summary(m)
```

```{r hedd, out.width = '50%', fig.align='center', fig.cap="(ref:hedd)"}
p.hedd
```

```{r diphthongs, out.width = '50%', fig.align='center', fig.cap="(ref:diphthongs)"}
p.diphthongs.average
```
...
Finally, the densities along the diagonal of Figure \@ref(fig:swe-vowels-all-cues) suggest that F0 carries the least information about vowel identity, exhibiting the least between-category separation, followed by F3. This, too, is not surprising: while some accounts use F0 to _normalize_ F1 and F2 [e.g., @miller1989c; @Syrdal1986], F0 is not considered an important cue to vowel identity by itself. <!-- TO DO: if we don't remove the pitch halving/doubling instances in the f0 data, we need to comment on them here. especially, the halving instances are quite clearly visible in the plot -->
...
```{r swe-vowels, out.width = '100%', fig.align='center', fig.cap="(ref:swe-vowels)"}
d.SwehVd %>% 
...
```{r swe-vowels-all-cues, fig.width=base.width*5, fig.height=base.height*5+.5, fig.cap="(ref:swe-vowels-all-cues)"}
p.matrix.cues <- 
  d.SwehVd %>% 
  filter(Transcribed_vowel == "targeted", Word != "hädd") %>%
...
(ref:swe-vowels-normalized-long) The Central Swedish long vowels in the unnormalized and normalized test data used in Study I. Points show all tokens of each of the 11 Central Swedish long vowels included in the test data in F1-F2 space. Top: vowels transformed to different perceptual scales. Middle: vowels in all scale-transformations further normalized to C-CuRE. Bottom two: vowels in six different normalization spaces.

(ref:swe-vowels-normalized-short) The Central Swedish short vowels in the unnormalized and normalized test data used in Study I. Points show all tokens of each of the 10 Central Swedish short vowels included in the test data in F1-F2 space. Top: vowels transformed to different perceptual scales. Middle: vowels in all scale-transformations further normalized to C-CuRE. Bottom two: vowels in six different normalization spaces.
...
p.vowels.norm.long <- 
  ...
  theme(
    ...
    theme(
      ...
      axis.text.y = element_text(size=10, hjust=1, vjust=.5),
      axis.title.x = element_text(size=10, vjust=0, hjust=0.5, face = "bold"),
      axis.title.y = element_text(size=10, hjust= 0.5, vjust=0.5, face = "bold"),
...
```{r swe-vowels-normalized-long, fig.width=base.width * 3 + 5, fig.height=base.height * 3, fig.align='center', fig.cap="(ref:swe-vowels-normalized-long)"}
p.vowels.norm.long
```
...
\begin{landscape}
```{r swe-vowels-normalized-short, fig.width=base.width *3 + 5, fig.height=base.height *3, fig.align='center', fig.cap="(ref:swe-vowels-normalized-short)"}
p.vowels.norm.short
```
...
Figure \@ref(fig:separability) shows the separability index for the normalization accounts investigated under the different assumptions about the relevant cues (Panel A-C) and depending on whether separability was calculated for separately for the long and short vowels or together. We make the following observations. (1) Across evaluations, accounts that center against the talker's overall formant mean, such as C-CuRE [@mcmurray-jongman2011] or @nearey1978 (in blue), are among the highest performing normalization accounts. No matter the assumed perceptual scale, centering always improves category separability (compare transformation accounts, in grey, normalized with C-CuRE, in blue). (2) The performance of what has been considered high-performing accounts in previous studies, such as @gerstman1968 and @lobanov1971 (in pink), varies as more cues are considered. Considering F1-F2, the results overall replicate previous findings [e.g., @disner1980; @fabricius2009; @Flynn2011]. When including F3, and further vowel duration, the same accounts perform considerably worse, with a separability index even lower than unnormalized cues. This does not replicate previous findings where F3 has been included [e.g., @adank2004]. However, these studies used a different effect measure. It might be the case that the inclusion of additional cues that display large between-talker variability but rather small between-category variance, such as F0, generates lower separability indices.<!--QUESTION FROM AP: why is this more so for Lobanov than the others? I don't really understand this?-->. (3) Some transformations to perceptual scales compare quite well with other accounts across evaluations, e.g. Bark. They do not outperform normalization accounts, but they do rank as good as some accounts, e.g. @miller1989c This replicates previous findings <!-- fill in here--> [@REFS]. (4) The separability index varies between long and short vowels for some normalization accounts. In general, category separability is higher for the short vowels across evaluations. Given that the short vowels form a more centralized and dense space, this result might appear puzzling. However, as indicated in Figure \@ref(fig:swe-vowels), the long vowels are more overlapping than the short, especially in F1-F2 space. Overall, the separability indices more or less aligns with the results from the visual inspection of scatter reduction introduced by applying normalization accounts (see Figures \@ref(fig:swe-vowels-normalized-long) and \@ref(fig:swe-vowels-normalized-short)). For instance, short vowels are visually more separated under C-CuRE comparing to the long vowels <!-- fill in here--> (5) As more cues are included, the separability index goes down. It might appear counter-intuitive that more information reduces performance, but<!-- fill in here--> The pattern with higher indices for the short vowels persist across evaluations. (6) (5) is also true when including vowel duration as additional cue. (7) Evaluating effects on category separability for all vowels together does not seem to make much difference. <!-- relate back to matrix plot?!-->Comparing to previous work, assessing effect of normalization on category variability using F-statistics (e.g., @labov2010), we note that <!--fill in here-->the same top performing accounts for F1 and F2 are <!-- fill in here-->. However, since vowel duration has not been evaluated as cue...

<!-- why?--> <!-- the adding of information/a dimension to the separability index for which there is a lot of inter-talker variability but that does not necessarily aid in categorization, might just provide the index with irrelevant information. This parameter adds substantial within-talker variation, which leads to the between-talker variance going up (since F0 adds variability between talkers but not between categories), which in turn pulls down the separability index. Might be more informative to exclude F0? Might be a misleading cue in this sense.-->
...
d.SwehVd.forStudy.SI.long <- 
  ...
  select(c(Normalization.Type, crossvalidation_group, Quantity, SI_F1F2, SI_F1F3, SI_all)) %>%
  pivot_longer(
...
```{r}
# Compare the estimates for the means and the sd across the five folds, check if the five folds in the test data have different sd
debug.sd.crossval <- d.SwehVd.forStudy.long %>%
  select(Talker, category, Token, Normalization.Type, crossvalidation_group, formants_mean, formants_sd) %>%
  mutate(
    F0_mean = sapply(formants_mean, "[[", 1), 
    F1_mean = sapply(formants_mean, "[[", 2),
    F2_mean = sapply(formants_mean, "[[", 3), 
    F3_mean = sapply(formants_mean, "[[", 4),
    F0_sd = sapply(formants_sd, "[[", 1), 
    F1_sd = sapply(formants_sd, "[[", 2),
    F2_sd = sapply(formants_sd, "[[", 3), 
    F3_sd = sapply(formants_sd, "[[", 4)) %>%
  select(-c(formants_mean, formants_sd)) %>%
  group_by(crossvalidation_group) %>%
  summarise(across(starts_with("F"), mean))

debug.crossval <- d.SwehVd.forStudy.long %>%
  select(Talker, category, Token, Normalization.Type, fold_type, crossvalidation_group, formants_mean, formants_sd) %>%
  mutate(
    F0_mean = sapply(formants_mean, "[[", 1), 
    F1_mean = sapply(formants_mean, "[[", 2),
    F2_mean = sapply(formants_mean, "[[", 3), 
    ...
    F3_sd = sapply(formants_sd, "[[", 4)) %>%
  select(-c(formants_mean, formants_sd)) %>%
  group_by(crossvalidation_group, fold_type) %>%
  summarise(across(starts_with("F"), mean)) %>%
  arrange(fold_type, crossvalidation_group)

debug.Talker <- d.SwehVd.forStudy.long %>%
  select(Talker, category, Token, Normalization.Type, fold_type, formants_mean, formants_sd) %>%
  mutate(
    F0_mean = sapply(formants_mean, "[[", 1), 
    F1_mean = sapply(formants_mean, "[[", 2),
    F2_mean = sapply(formants_mean, "[[", 3), 
    ...
    F3_sd = sapply(formants_sd, "[[", 4)) %>%
  select(-c(formants_mean, formants_sd)) %>%
  group_by(Talker, fold_type) %>%
  summarise(across(starts_with("F"), mean)) %>%
  pivot_longer(
    cols = starts_with("F", ignore.case = FALSE),
    names_to = c("Formant", "measure"),
    names_sep = "_",
    values_to = "Cue.Value") %>%
  pivot_wider(
    names_from = "fold_type",
    values_from = "Cue.Value") %>%
  mutate(
    relative_measure = case_when(measure == "sd" ~ test / training,
                                 measure == "mean" ~ test - training))

debug.Talker.2 <- d.SwehVd.forStudy.long %>%
  select(Talker, category, Token, Normalization.Type, fold_type, F0, F1, F2, F3) %>%
  #filter(fold_type == "test") %>%
  group_by(Talker, Normalization.Type, fold_type) %>%
  summarise(across(starts_with("F"), .fns = list("mean" = mean, "sd" = sd), na.rm = T)) %>%
  pivot_longer(
    cols = starts_with("F", ignore.case = FALSE),
    names_to = c("Formant", "measure"),
    names_sep = "_",
...
debug.Talker.2 %>%
  filter(Normalization.Type == "no normalization (Hz)") %>%
  left_join(d.SwehVd.forStudy.long %>%
    select(Talker, category, Token, Normalization.Type, formants_mean, formants_sd) %>%
    mutate(
      F0_mean_params = sapply(formants_mean, "[[", 1), 
      F1_mean_params = sapply(formants_mean, "[[", 2),
      F2_mean_params = sapply(formants_mean, "[[", 3), 
      F3_mean_params = sapply(formants_mean, "[[", 4),
      F0_sd_params = sapply(formants_sd, "[[", 1), 
      F1_sd_params = sapply(formants_sd, "[[", 2),
      F2_sd_params = sapply(formants_sd, "[[", 3), 
      F3_sd_params = sapply(formants_sd, "[[", 4)) %>%
    select(-c(formants_mean, formants_sd)) %>%
    group_by(Talker) %>%
    summarise(across(starts_with("F"), mean)) %>%
    pivot_longer(
      ...
      names_sep = "_",
      values_to = "Cue.Value"),
    by = c("Talker", "Formant", "measure")) %>%
  mutate(
    relative_measure_by_params = case_when(measure == "sd" ~ test / Cue.Value,
                                           measure == "mean" ~ test - Cue.Value)) %>%
  select(-c(test, training, relative_measure, Cue.Value)) %>%
  pivot_wider(
    names_from = "measure",
    values_from = "relative_measure_by_params") %>%
  ggplot(
    aes(
      x = mean,
      y = sd,
  ...
  scale_x_continuous("relative mean (test-training)") +
  scale_y_continuous("relative sd (test/training)") +
  facet_grid(~Formant)

debug.Talker %>%
  select(-c(test, training)) %>%
  pivot_wider(
    names_from = "measure",
    values_from = "relative_measure") %>%
  ggplot(
    aes(
      x = mean,
      y = sd,
      shape = Talker, color = Talker)) +
  geom_point() +
  scale_shape_manual(values = 0:16) +
  scale_x_continuous("relative mean (test-training)") +
  scale_y_continuous("relative sd (test/training)") +
...
```{r}
#Apply normalization procedures in the old functions
d.debug.norm %<>%
  group_by(Talker) %>%
  mutate(
    F0_Hz_Lobanov = normLobanov(F0),
    F1_Hz_Lobanov = normLobanov(F1),
    F2_Hz_Lobanov = normLobanov(F2),
    F3_Hz_Lobanov = normLobanov(F3),
    Duration_Hz_Lobanov = normLobanov(Duration),
    F0_gm = geometric.mean(F0),
    F0_log_Miller = log(F0),
    F1_log_Miller = F1_to_height(F1 = F1, SR = 168 * (F0_gm / 168) ^ (1/3)),
    F2_log_Miller = F2_to_backness(F1, F2),
    F3_log_Miller = log(F3 / F2),
    Duration_log_Miller = log(Duration)) %>%
  rename(F0_Hz_r = F0, F1_Hz_r = F1, F2_Hz_r = F2, F3_Hz_r = F3, Duration_Hz_r = Duration)

d.debug.norm %<>%
  select(-F0_gm) %>%
  pivot_longer(
    cols = c(starts_with("Duration"), starts_with("F", ignore.case = FALSE)),
    names_to = c("Cue", "Normalization.Scale", "Normalization.Type"),
    names_sep = "_",
    values_to = "Cue.Value") %>%
  pivot_wider(
...
# debug <- d.SwehVd.forStudy.long %>% 
#   filter(Normalization.Type %in% c("no normalization (Hz)", "SyrdalGopal (Bark)", "Lobanov (Hz)"))
# 
# debug[rowSums(is.na(debug)) > 0, ]
# 
# debug.SI <- d.SwehVd.forStudy.SI %>%
#    filter(Normalization.Type %in% c("no normalization (Hz)", "SyrdalGopal (Bark)", "Lobanov (Hz)"))
# 
# debug.SI %>%
#   group_by(Normalization.Type, crossvalidation_group) %>%
#   summarise_at(vars(contains("df")), mean)
#  
# debug.SI.withinMS <- d.SwehVd.forStudy.long.within_category_MS %>%
#    filter(Normalization.Type %in% c("no normalization (Hz)", "SyrdalGopal (Bark)", "Lobanov (Hz)"))
# 
# d1 <- debug.SI.withinMS %>%
#   group_by(Normalization.Type, category) %>%
#   summarise_at(vars(contains("df")), mean)
# 
# debug.SI.withinMS[rowSums(is.na(debug.SI.withinMS)) > 0, ]
```


```{r}
#DOING THE OLD CODE JUST TO RULE OUT SI CODE ERROR --------------------------------------------------
# d.Swe.withincat.MS <- d.SwehVd.forStudy.long %>%
#     filter(fold_type == "test") %>%
#     # Get within category MS and pseudo-MS
#   group_by(Normalization.Type, category, crossvalidation_group) %>% 
#   summarise(
#     # First obtain df and SS for each cues
#     across(
#       .cols = c(F0, F1, F2, F3, Duration), 
#             .fns = list(
#         "df" = ~ sum(!is.na(.x)) - 1, 
#         "mean" = ~ mean(.x, na.rm = T)), 
#          .names = "{.fn}_{.col}"),
#         # Since f1 and f2 are always observed, and thus there are equally many f1 and f2 values, we only need one df
#     SS_F0 = sum((F0 - mean_F0)^2, na.rm = T),
#     pseudo_MS_F0 = SS_F0 / (df_F0),
#     SS_F1 = sum((F1 - mean_F1)^2, na.rm = T),
#     pseudo_MS_F1 = SS_F1 / (df_F1), 
#     SS_F2 = sum((F2 - mean_F2)^2, na.rm = T),
#     pseudo_MS_F2 = SS_F2 / (df_F2),
#     SS_F3 = sum((F3 - mean_F3)^2, na.rm = T),
#     pseudo_MS_F3 = SS_F3 / (df_F3),
#     SS_Duration = sum((Duration - mean_Duration)^2, na.rm = T),
#     pseudo_MS_Duration = SS_Duration / (df_Duration),
#     # Since f1 and f2 are always observed, and thus there are equally many f1 and f2 values, the SS F1F2 is just the sum of the SS_F1 and SS_F2
#     SS_F1F2 = SS_F1 + SS_F2,
#     pseudo_MS_F1F2 = SS_F1F2 / (df_F1),
#     SS_F1F3 = SS_F1 + SS_F2 + SS_F3,
#     pseudo_MS_F1F3 = SS_F1F3 / (df_F1),
#     SS_F0F3Dur = SS_F0 + SS_F1 + SS_F2 + SS_F3 + SS_Duration,
#     pseudo_MS_F0F3Dur = SS_F0F3Dur / (df_F1))
#   
# d.Swe.separability.long <- d.Swe.withincat.MS %>%
#   filter(category %in% levels.vowel.IPA.swe.long) %>%
#     group_by(Normalization.Type, crossvalidation_group) %>%
#   summarise(
#     within_MS_F0 = sum(SS_F0, na.rm = T) / sum(df_F0, na.rm = T),
#     within_MS_F1 = sum(SS_F1, na.rm = T) / sum(df_F1, na.rm = T),
#     within_MS_F2 = sum(SS_F2, na.rm = T) / sum(df_F2, na.rm = T),
#     within_MS_F3 = sum(SS_F3, na.rm = T) / sum(df_F3, na.rm = T),
#     within_MS_Duration = sum(SS_Duration, na.rm = T) / sum(df_Duration, na.rm = T),
#     within_MS_F1F2 = sum(SS_F1F2, na.rm = T) / sum(df_F1, na.rm = T),
#     within_MS_F1F3 = sum(SS_F1F3, na.rm = T) / sum(df_F1, na.rm = T),
#     within_MS_F0F3Dur = sum(SS_F0F3Dur, na.rm = T) / sum(df_F1, na.rm = T)) %>%
#     left_join(
#     d.SwehVd.forStudy.long %>%
#         filter(fold_type == "test") %>%
...
#       group_by(Normalization.Type, crossvalidation_group) %>% 
#       mutate(
#         overall_mean_F0 = mean(F0, na.rm = T),
#         overall_mean_F1 = mean(F1, na.rm = T),
#         overall_mean_F2 = mean(F2, na.rm = T),
#         overall_mean_F3 = mean(F3, na.rm = T),
#         overall_mean_Duration = mean(Duration, na.rm = T)) %>% 
#             group_by(Normalization.Type, category, crossvalidation_group) %>%
#       # Get category means and carry through overall mean
#       summarise(
#         across(
...
#                 first), 
#         mean_F0 = mean(F0, na.rm = T),
#         mean_F1 = mean(F1, na.rm = T),
#         mean_F2 = mean(F2, na.rm = T),
#         mean_F3 = mean(F3, na.rm = T),
#         mean_Duration = mean(Duration, na.rm = T),
#         mean_F1F2 = (mean(F1, na.rm = T) + mean(F2, na.rm = T)) / 2,
#         mean_F1F3 = (mean(F1, na.rm = T) + mean(F2, na.rm = T) + mean(F3, na.rm = T)) / 3,
#         mean_F0F3Dur = (mean(F0, na.rm = T) + mean(F1, na.rm = T) + mean(F2, na.rm = T) + mean(F3, na.rm = T) + mean(Duration, na.rm = T)) / 5) %>%
#       # Get between MS
#             group_by(Normalization.Type, crossvalidation_group) %>%
#       summarise(
#         between_MS_F0 = sum((mean_F0 - overall_mean_F0)^2, na.rm = T) / (sum(!is.na(mean_F0)) - 1),
#         between_MS_F1 = sum((mean_F1 - overall_mean_F1)^2, na.rm = T) / (sum(!is.na(mean_F1)) - 1),
#         between_MS_F2 = sum((mean_F2 - overall_mean_F2)^2, na.rm = T) / (sum(!is.na(mean_F2)) - 1),
#         between_MS_F3 = sum((mean_F3 - overall_mean_F3)^2, na.rm = T) / (sum(!is.na(mean_F3)) - 1),
#         between_MS_Duration = sum((mean_Duration - overall_mean_Duration)^2, na.rm = T) / (sum(!is.na(mean_Duration)) - 1),
#         between_MS_F1F2 = sum((mean_F1 - overall_mean_F1)^2 + (mean_F2 - overall_mean_F2)^2, na.rm = T) / (sum(!is.na(mean_F1) & !is.na(mean_F2)) - 1),
#         between_MS_F1F3 = sum((mean_F1 - overall_mean_F1)^2 + (mean_F2 - overall_mean_F2)^2 + (mean_F3 - overall_mean_F3)^2, na.rm = T) / (sum(!is.na(mean_F1) & !is.na(mean_F2) & !is.na(mean_F3)) - 1),
#         between_MS_F0F3Dur = sum((mean_F0 - overall_mean_F0)^2 + (mean_F1 - overall_mean_F1)^2 + (mean_F2 - overall_mean_F2)^2 + (mean_F3 - overall_mean_F3)^2 + (mean_Duration - overall_mean_Duration)^2, na.rm = T) / (sum(!is.na(mean_F0) & !is.na(mean_F1) & !is.na(mean_F2) & !is.na(mean_F3) & !is.na(mean_Duration)) - 1)),
#     by = c("Normalization.Type", "crossvalidation_group")) %>%
#     # Now get the separability index
#   mutate(
#     SI_F1F2 = between_MS_F1F2 / within_MS_F1F2,
#     SI_F1F3 = between_MS_F1F3 / within_MS_F1F3,
#     SI_F0F3Dur = between_MS_F0F3Dur / within_MS_F0F3Dur) %>%
#   mutate_if(is.numeric, round, digits = 4)
# 
# d.Swe.separability.short <- d.Swe.withincat.MS %>%
#    filter(category %in% levels.vowel.IPA.swe.short) %>%
#   group_by(Normalization.Type, crossvalidation_group) %>%
#   summarise(
#     within_MS_F0 = sum(SS_F0, na.rm = T) / sum(df_F0, na.rm = T),
...
#   mutate_if(is.numeric, round, digits = 4)
# 
# d.Swe.separability.F0.F3.dur <- d.Swe.withincat.MS %>%
#   group_by(Normalization.Type, crossvalidation_group) %>%
#   summarise(
#     within_MS_F0 = sum(SS_F0, na.rm = T) / sum(df_F0, na.rm = T),
#     within_MS_F1 = sum(SS_F1, na.rm = T) / sum(df_F1, na.rm = T),
...
#         between_MS_Duration = sum((mean_Duration - overall_mean_Duration)^2, na.rm = T) / (sum(!is.na(mean_Duration)) - 1),
#         between_MS_F1F2 = sum((mean_F1 - overall_mean_F1)^2 + (mean_F2 - overall_mean_F2)^2, na.rm = T) / (sum(!is.na(mean_F1) & !is.na(mean_F2)) - 1),
#         between_MS_F1F3 = sum((mean_F1 - overall_mean_F1)^2 + (mean_F2 - overall_mean_F2)^2 + (mean_F3 - overall_mean_F3)^2, na.rm = T) / (sum(!is.na(mean_F0) & !is.na(mean_F1) & !is.na(mean_F2) & !is.na(mean_F3)) - 1),
#         between_MS_F0F3Dur = sum((mean_F1 - overall_mean_F0)^2 + (mean_F1 - overall_mean_F1)^2 + (mean_F2 - overall_mean_F2)^2 + (mean_F3 - overall_mean_F3)^2 + (mean_Duration - overall_mean_Duration)^2, na.rm = T) / (sum(!is.na(mean_F0) & !is.na(mean_F1) & !is.na(mean_F2) & !is.na(mean_F3) & !is.na(mean_Duration)) - 1)),
#     by = c("Normalization.Type", "crossvalidation_group")) %>%
#   # Now get the separability index
#   mutate(
#     SI_F1F2 = between_MS_F1F2 / within_MS_F1F2,
...
#   mutate_if(is.numeric, round, digits = 4)
# 
# p.separability.F1F2.long <- d.Swe.separability.F0.F3.dur %>%
#   ggplot(
...
#   theme(
#     axis.text.x = ggtext::element_markdown(angle = 45, vjust = 1, hjust=1, colour = colors.all.procedures),
...
p.separability.F1F2_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "F1F2", Quantity != "all vowels")) + facet_grid(~ Quantity)
p.separability.F1F3_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "F1F3", Quantity != "all vowels")) + facet_grid(~ Quantity)
p.separability.all_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "all", Quantity != "all vowels")) + facet_grid(~ Quantity)
...
cowplot::plot_grid(
  ...
  labels = c("A", "B", "C"), nrow = 3, align = "hv", scale = 0.9)
```

# Study II {#sec:studyII}
In Study I, we evaluated the extent to which normalization removes category overlap due to inter-talker variability. In Study II, we assess the anticipated effects of normalization for vowel perception, by investigating how normalization accounts are predicted to affect the accuracy with which a vowel would be perceived. We ask whether normalization models differ from unnormalized models in predicting vowel categorization, and if so, which normalization model makes the strongest predictions for perception. Models are assessed under the same assumptions about the relevant cues employed when evaluating the effects on category separability (see Section \@ref(sec:cues)), with the addition of F0 to the final evaluation.
...
<!-- Moved from Introduction since last meeting Oct 27-->
<!-- TO DO this might all be able to go, but let's go through it during a meeting: Certain measures of changes in variability can be assumed to assess consequences for perception, e.g., distance to nearest neighbour or category separability, given that most theories of speech perception would predict that both reduced distance between neighbouring category means and increased variability leading to category overlap, would result in reduced categorization accuracy (see e.g. REF). However, measures of category separability are not often used in comparisons of normalization accounts, and if they are, they do not always consider the entire vowel system (REF).  If normalization accounts are meant to reflect the actual perceived vowel quality, i.e. how humans perceptually adapt to inter-talker variability, a measure of reduced variability should consider the *system level consequences of normalization* -- that is, how reduced variability affects the entire vowel system of a language. If we use an index of overall variability that does not assess the category separability of the entire vowel system, we might observe a reduction in overall variability after normalization that is caused by all vowels separately shrinking, but that leaves the relative variability of each vowel in comparison to the others in the space unchanged. In other words, the index might not display differences in the *relation* between categories in the space caused by normalization, but merely reflect a change in variance of each category separately. Whether reduced variance for each category separately has an effect on perception is not a given, neither is whether changes in category variability will be the most important source affecting perception overall. As noted in previous research, normalization models that perform well at scatter reduction does not necessarily provide the best fit to human perception (REF). A focus on variability reduction might also disregard other mechanisms supporting speech perception beyond normalization, such as talker-specific learning. A comparison of normalization accounts should thus not only evaluate variability effects at the vowel system level by considering reduction in variability, but also model perceptual effects in terms of perceived vowel quality. 

The purpose of this study is therefore to investigate the robustness of the effect of different normalization accounts on removing inter-talker variability in vowel data, and predict the effects of these accounts on the perception of Swedish vowels. We will assess this based on Swedish vowel production data in two studies.

In Study II, we add to the few existing studies relating the consequences of normalization to perception [see e.g. @barreda2018a; @escudero2007; @nearey1989; @richter2017], but also expand on previous research, by using a computational model that allows us to assess the consequences of normalization for perception while considering the entire vowel system. We use a computational model based on Bayesian inference, ideal observers (IO; REF), to simulate the effects and evaluate the predicted consequences of normalization accounts for speech perception from production data.-->

Previous studies have related the consequences of normalization to predicted perception using a variety of different measures. Most previous work have either used more indirect approaches, such as comparing the distance between talker-specific category means for unnormalized and normalized vowel data [e.g., @barreda2018a; @lobanov1971]<!-- AP: is this necessary to bring up here? B&N does this whole analysis of suitablility of approaches as models of human cognition, where these measures are included, but it is relevant here?: calculating amount of error in normalization parameter estimates [e.g., @barreda2018a]-->, or more direct approaches essentially aiming at categorizing tokens [e.g., @adank2004; @barreda2018a; @carpenter1993; @escudero2007; @syrda1985]. Different models have been used in these studies, e.g., linear discriminant analysis [e.g., @adank2004; @syrda1985], fuzzy ARTMAP and K-nearest neighbour [e.g., @carpenter1993], or constraint ranking grammars [e.g., @escudero2007]. 
...
Figure \@ref(fig:predictions) visualizes the unnormalized and normalized models' predictions for perception of Stockholm Swedish vowels, under different assumptions about the relevant cues. The posterior probability of recognizing a certain vowel as intended by talkers, is displayed on the y-axis. 
Previous research assessing differences in normalization accounts' predictions for perception, have not investigated quantity distinctions, most investigations have used vowel inventories that can be considered as primarily comparable to the long vowels in our study [e.g., @adank2004; @barreda2018a; @escudero2007; @syrda1985]. With regard to our models' predictions for the long vowels, the following observations can be made based on Figure \@ref(fig:predictions), leftmost column. (1) All models perform substantially above chance (9.1% in an 11-way forced choice task). (2) Transformations from the acoustic space into perceptual spaces does not seem to improve the models' performance, but (3) normalization can. General statistical operations such as centering (e.g., Nearey, C-CuRE) and/or scaling (e.g., Lobanov) are the best performing models. Notably, general purpose accounts such as C-CuRE, perform as well as other centering accounts. The high performance of these normalization models (as well as the lower performance of scale transformation models) replicates previous findings that compares normalization against simulated responses [e.g., @adank2004; @barreda2018a; @escudero2007; @syrda1985]. (4) As more cues are included in the models (Figure \@ref(fig:predictions), from **Panel A** to **Panel C**), the general pattern persist, and in addition, the accuracy of the models in predicting the intended category increases. Of note is that the relative difference between centering and scaling accounts diminishes, especially when duration is included as cue (**Panel C**, Figure \@ref(fig:predictions)). Given that including more cues presumably makes the models more human-like, humans might potentially do equally well with a simple centering approach as with a simple scaling approach. Even if normalization in general improve categorization accuracy, (5) some normalization accounts do not perform better that perceptual scale models. This is true for the intrinsic normalization accounts [@miller1989c; @Syrdal1986]. Previous work have indicated similar results for Dutch vowels [@adank2004], contrary to some evaluations of English [e.g., @syrda1985]. @adank2004 speculated that the discrepancy in results might be attributed to implementations of the bark-transformation, or to what @syrda1985 describe as language-specificity of the second dimension of @Syrdal1986 normalization. It might thus not generate the same results for different languages. The low performance of @Syrdal1986 and @miller1989c might also be related to the fact that they do not seem to increase category separability much, in comparison to other accounts (see Section \@ref(sec:normVowelSpace)) <!-- AP: this might be changed, if SI changes.-->.

Points 1-5 hold for the models trained on the short vowels as well (Figure \@ref(fig:predictions), middle column). (6) The normalization accounts pattern the same, and in comparison to the long vowels, the categorization accuracy of the models predicting the intended category is now well above 75% (chance level being 10% in a 10-way forced-choice task). Here, we evaluate against a 10 category vowel space, thus one category less than in the previous evaluation. However, this increase in performance cannot be attributed to this difference alone, accuracy increase more than is expected based on the removal of one category. Given that phonological descriptions of Swedish vowels characterize the short vowels as being more centralized, constituting a smaller space [e.g., @riad2014], which is also confirmed by visualizations of the SwehVd (see Section \@ref(sec:characterizing-swehvd), this result might initially seem puzzling. However, the short vowels also exhibit less category variability and less category overlap, which presumably is driving this difference in performance between models trained on long and short vowels separately.

Finally, evaluating predictions for the entire vowel space of 21 categories (Figure \@ref(fig:predictions), right column), which is presumably the most complex problem, we see that the normalization accounts pattern the same. Lobanov is the best performing normalization model, but this advantage is substantially reduced when considering all cues. In comparison to the models trained on long and short vowels separately, these models perform quite poorly when trained on F1-F2 and F1-F3 cues alone, with an accuracy of around 50% or less. Fifty percent is of course still substantially above chance, now being at only 4.8% (in a 21-way forced choice task). When including duration in the final evaluation, however, the models achieve around 75% accuracy, which is almost as high as models trained on long and short vowels separately. This points to the importance of duration as cue for a complex vowel system with quantity distinctions, such as the Swedish vowel space.
...
```{r predictions, fig.width= base.width * 4, fig.height = base.height * 5.5, fig.align='center', fig.cap="(ref:predictions)"}
cowplot::plot_grid(
...
Considering the models' predictions for each vowel separately, the results overall hold. We will, however, make a couple of points in passing. For reference, Figure \@ref(fig:predictions-per-vowel) visualizes the predicted categorization accuracy per vowel, by the unnormalized and normalized IO models trained on F1-F2 (the SI contains similar plots for models trained on F1-F3, and all five cues, see <!-- insert reference here--> ). Even though the models pattern more or less the same way across vowels, there are some vowels for which models are overall good at predicting the intended category, across cue evaluations and across space sizes, e.g., [`r linguisticsdown::cond_cmpl("ɑː")`], [`r linguisticsdown::cond_cmpl("ɑ")`] and [`r linguisticsdown::cond_cmpl("ɵ")`]. Similarly, there are also some vowels for which model performance is specially low, in particular [`r linguisticsdown::cond_cmpl("iː")`], but also to a certain extent [`r linguisticsdown::cond_cmpl("oː")`] and [`r linguisticsdown::cond_cmpl("uː")`]. Of note is also that one of these vowels does not seem to benefit at all from normalization, across evaluations, [`r linguisticsdown::cond_cmpl("iː")`]. The unnormalized model perform similarly to normalized models for this vowel. Which vowels are aided by normalization will naturally vary between languages, reflecting the language-specific properties of the vowel space. While [`r linguisticsdown::cond_cmpl("iː")`] does not seem to benefit from normalization in Swedish, it is often described as more easily recognized in previous work on other languages. Its recognition accuracy is often attributed to either its absolute location in the space (peripheral vowel), or to the fact that it does not have close competitors. Given the density of the Swedish vowel space along the height dimension, vowels such as [`r linguisticsdown::cond_cmpl("iː")`] have, on the contrary, many close competitors, which might explain the performance of the IOs.
...
plot_io_results_per_vowel <- function(data) {
  data %>%
    ...
    guides(color = "none") +
    theme(
      axis.text.x = ggtext::element_markdown(angle = 90, vjust = 1, hjust = 1, colour = colors.all.procedures),
      axis.title.x = element_blank(),
      axis.title.y = element_blank())
}
...
```{r predictions-per-vowel, fig.width=base.width * 5 + 5, fig.height=base.height * 3, fig.align='center', fig.cap="(ref:predictions-per-vowel)"}
cowplot::plot_grid(
...
The approach that we have taken here lets us assess vowel-specific effects on the predictions of the IOs. This is however not the main focus of this paper, but might be of interest to other researchers. In SI (\@ref(sec:confusion), we therefore plot confusion matrices of the best and the worst performing IOs, to further investigate the beliefs IOs hold about the Swedish vowel space, in terms of which vowel categories are more easily confused, and with what. Here, we briefly mention some of the main general points, illustrated by Figure \@ref(fig:confusion-matrix) that displays the models' predictions about the entire Swedish vowel space. (1) When there is high competition from many close neighbours, models are more inclined to confuse categories with one of there neighbours. This is especially evident among the overlapping high vowels, but also for the allophones to [`r linguisticsdown::cond_cmpl("ɛ")`] and [`r linguisticsdown::cond_cmpl("ø")`]. (2) Given that the closest neighbour is not necessarily the categories' short/long counterpart, it is not primary the case that quantity distinctions are the cause of models' confusion. (3) As more information is given the models, the confusion declines substantially (panel B, Figure \@ref(fig:confusion-matrix)). Including F3 as cue reduces the confusion among some of the high vowels, including duration reduces quantity confusions down to one category distinction (between [`r linguisticsdown::cond_cmpl("uː")`] and [`r linguisticsdown::cond_cmpl("ʊ")`] for the worst performing model). 

In sum, these results might suggest that even if duration is cue to quantity distinctions between long and short vowels in Stockholm Central Swedish, successful vowel recognition when all vowels are taken into account, most likely relies on more cues.
...
```{r confusion-matrix, fig.width=base.width*3, fig.height=base.height*3+.5, fig.align='center', fig.cap="(ref:confusion-matrix)"}
cowplot::plot_grid(legend.confusion,
  cowplot::plot_grid(
    cowplot::plot_grid(p.confusion.F1F2.all.worst, p.confusion.F1F2.all.best),
    cowplot::plot_grid(p.confusion.all.all.worst, p.confusion.all.all.best + theme(legend.position = "none")), align = "hv", axis = "btlr", labels = c("A", "B"), ncol = 1),
  nrow = 2, rel_heights = c(.08,1))
```

# General discussion {#sec:discussion}
This paper has investigated the effect of low-level pre-linguistic normalization on Stockholm Swedish vowels, by assessing the effects on category separability, and on predicted consequences for perception. The evaluation of the 15 different normalization accounts against the dense vowel space of Swedish, has allowed us to assess whether the same normalization accounts that work well for vowel systems with fewer categories, work equally well for more complex systems. Investigating a language with quantity distinction has allowed for a bridging of the literature on vowel-specific accounts and on general normalization accounts [such as C-CuRE; @mcmurray-jongman2011], and for expanding on previous research that primarily has focused on the first two (or three) formants. 

The speech materials used was a subset of the SwehVd database, a new phonetically annotated database of Stockholm Swedish vowels that was presented and released in this paper. The following methods were used for evaluation. In Study I, we compared the reduction of within-category variability relative to between-category variability by calculating a separability index (i.e., an F-statistics). Using this type of metric facilitates comparison to previous studies.<!--AP: more here ---> In Study II, we assessed the predicted consequences for perception by training ideal observer models on the vowel data in different normalization spaces, under different assumptions about the relevant cues. This approach comes with a set of simplifying assumptions: for instance, we assume that categories can be represented as Gaussian distributions by their means and covariance matrices, we furthermore assume uniform category priors... All of these assumptions can, however, be revisited as code is provided in the SI/OFS... The advantage of the model in having 0 degrees of freedom, strikes a middle ground between less parsimonious models such as exemplar models, and more parsimonious, such as models of cue integration...<!-- AP: more here on modelling approach-->

The results from Study I showed that the same type of accounts that have been proven successful in previous evaluations, are also successful in improving category separability on Swedish vowels. The increase in category separability for the best performing normalization accounts, seems to be equally large as those reported by @disner1980 for the other languages in her cross-linguistic study.<!-- AP: go back to Disner here--> Furthermore, here, simple centering performs as good, or even better, for Swedish as it did for the other languages in her study. In conclusion, it might well be the case that the (somewhat) unexpected/diverging <!-- change word--> results for Swedish in @disner1980's study, could be an effect of the nature of the dataset used <!-- AP: or male vs female--> However, ....<!-- AP: check results om Labov, Barreda/Nearey, that also used Fstats-->. 

Study II indicated that psycho-acoustic scale transformations alone does not make as strong predictions for perception as normalization accounts do. Lobanov and centering approaches such as Nearey or C-CuRE, are the top-performing accounts, which replicates previous studies comparing predictions against simulated data [@REF]. <!-- AP: more here relating to previous studies--> Simple centering performs very well, especially when more cues are included, making models presumably more human-like. The relative advantage of Lobanov normalization seems to decrease as more cues are included, possible indicating that simple centering might make up a plausible candidate for a mechanism supporting robust inter-talker perception. More robust evidence for such a claim, would of course be to compare models against perceptual data. This is also our intention, moving forward... 

To summarise, comparing normalization accounts against Swedish has showed that normalization seem to work equally well for more crowded vowel systems. By evaluating accounts using more cues, shows that general accounts that are not specific to phonological categories or phonetic cues, work as good as other accounts...

  * Its plausibility as a model for robust inter-talker perception...
  * More future: train IOs on the onfolding signal, so at all timepoints?
  * keep in mind, some talkers have less data.
  * keep in mind, all of them are female.
  

<!-- a note on the normalization accounts' plausibility as models of human speech perception-->
scale transformations are often suggested to provide a simpler architecture as normalization is performed formant-wise within each vowel. In order to normalize across talkers, a listener would thus not need more input than a single vowel from an unfamiliar talker [@nearey1989]. Furthermore, scale transformations that approximate log-transforming frequency are often presented as providing a more accurate reflection of the psychoacoustics of speech perception and the human brain's representation of frequency information.

Since they require estimates of more parameters, and/or input from a talker's entire vowel space, they represent a more complex architecture. 
More importantly, @barreda2018a claimed that even though Lobanov performed better at scatter reduction, it constitutes an implausible model of human speech perception because of (1) its complexity, (2) its tendency to over-normalize and (3) its sensitivity to the amount of data. Concerning the last two points, @barreda2018a argued that @disner1980's concern with the use of normalization accounts for cross-linguistic comparisons was essentially applicable to any situation with missing or unbalanced data, since it would necessarily introduce inventory difference bias, that could generate artifacts. Regarding the first point, given that Lobanov normalization requires estimation of two parameters for every formant, @barreda2018a questioned the plausibility of the speech perception system correctly estimating the required number of independent parameters from a single vowel (as is hypothetically the case when understanding an unfamiliar talker). The authors further claimed that the fact that many common normalization accounts recode a talker's formants into a talker-specific score by using the talker's entire vowel space (e.g. Nearey1, Lobanov, Gerstman) makes them quite implausible as cognitive models of perception. @barreda2018a argued for the fact that "normalization accounts should be approached not as models of variation in formant patterns between talkers, but rather as models of human perceptual accommodation to this variation". If evidence suggest that normalization accounts might require mathematical computations unlikely to reflect human cognition, the authors suggest reconsidering whether they should be used at all.
...
## Per-vowel categorization accuracy of models trained on long and short vowels separately
...
```{r correlation-matrix-Mel, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-Mel)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-Bark, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-Bark)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-ERB, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-ERB)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-semitones, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-semitones)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-ccure-hz, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-ccure-hz)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-ccure-mel, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-ccure-mel)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-ccure-bark, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-ccure-bark)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-ccure-erb, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-ccure-erb)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-ccure-semitones, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-ccure-semitones)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-nearey1, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-nearey1)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-nearey2, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-nearey2)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-syrdalgopal, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-syrdalgopal)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-miller, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-miller)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-gerstman, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-gerstman)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
```{r correlation-matrix-lobanov, fig.width=base.width * 5, fig.height=base.height * 5 + .5, fig.cap="(ref:correlation-matrix-lobanov)"}
p.matrix.cues %+%
  (d.SwehVd.forStudy.long %>% 
...
d.outliers %>%
  ...
  theme(axis.text.x = element_text(size=7, vjust=1),
...
```{r confusion-matrix-long, fig.width= base.width * 2, fig.height = base.height * 3, fig.align='center', fig.cap="(ref:confusion-matrix-long)"}
cowplot::plot_grid(legend.confusion,
  cowplot::plot_grid(
...
```{r confusion-matrix-short, fig.width= base.width * 2, fig.height = base.height * 3, fig.align='center', fig.cap="(ref:confusion-matrix-short)"}
cowplot::plot_grid(legend.confusion,
  cowplot::plot_grid(
...
```{r confusion-matrix-allVowels, fig.width= base.width * 2, fig.height = base.height * 3, fig.align='center', fig.cap="(ref:confusion-matrix-allVowels)"}
cowplot::plot_grid(legend.confusion,
  cowplot::plot_grid(
    cowplot::plot_grid(p.confusion.F1F2.all.worst, p.confusion.F1F2.all.best),
    cowplot::plot_grid(p.confusion.F1F3.all.worst, p.confusion.F1F3.all.best),
    cowplot::plot_grid(p.confusion.all.all.worst, p.confusion.all.all.best + theme(legend.position = "none")), align = "hv", axis = "btlr", labels = c("A", "B", "C"), ncol = 1),
  nrow = 2, rel_heights = c(.08,1))
```
