...
```{r}
#Select points
#Use the above function at e.g., level = .9. All points it'll generate will be on the 80th quantile (or 80% highest density interval, HDI), or whatever value you want. Segments = 4 should suffice:
maaaa_points <- get_bivariate_normal_ellipse(mu = unlist(d.two_sim_vowels$mu[2]), Sigma = matrix(unlist(d.two_sim_vowels$Sigma[2]), ncol = 2), level = .9, segments = 4)

# have a look
#plot(maaaa_points)
#Select the two points with the most extreme F2-values 
# maaaa_points %>%
#   filter(F2 %in% range(F2))
