...
All data and code for this article can be downloaded from OSF at <https://osf.io/zb8gx/>. This article is written in R markdown, allowing readers to replicate our analyses using freely available software [@R; @RStudio], while changing any of the parameters of our models. Readers can revisit and alter the assumptions we make---for example, categorization model, models of linguistic representations, the normalization accounts selected. The supplementary information (SI) lists the software/libraries required to compile this document.

# Methods and materials {#sec:methodsII -}
In this section, we begin by introducing the new phonetically annotated corpus of Central Swedish vowel productions employed in the present study. We then present the perceptual model that we use for assessing the predicted effects of different normalization accounts---a Bayesian ideal observer.
