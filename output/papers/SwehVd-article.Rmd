...
# To Do
  ...
  * Lift out corpus description and adjust paper accordingly.
  * Add REFS for footnote on noise.
  * FIGs
...
```{r illustrate-normalization, fig.width=base.width*2.5, fig.height=base.height*3.5+.5, out.width='80%', fig.align='center', fig.cap="(ref:illustrate-normalization)"}
cowplot::plot_grid(
  legend,
  cowplot::plot_grid(
...
We compare the normalization accounts in Table \@ref(tab:norm-accounts) both in terms of their effectiveness in reducing within-category variability relative to between-category variability (Study 1) and in terms of the predicted consequences for recognition accuracy (Study 2). Study 1 evaluates the effect of normalization by visual inspection and a separability index, which are common approaches in previous studies. The primary interest of this paper is to assess the predicted consequences of normalization for perception. For this we take a second approach that more directly addresses this, while making additional assumptions that Study 1 avoids. In hindsight, this turned out to be of importance since we revealed an empirical issue we had not anticipated, namely, that the separability index can be very sensitive to one of the cue dimensions included.

Following the bulk of previous studies [but see e.g., @nearey1989; @richter2017], the present studies does not compare the predicted consequences of normalization against categorization responses from human listeners, though we plan to do so in future work. In both studies, we compare normalization accounts applied to (1) only F1 and F2, as in the majority of previous studies, (2) F1-F3, as in, e.g., @adank2004, and (3) F0-F3 as well as vowel duration. This allows us to assess whether differences in the effectiveness of normalization accounts depend on the number and types of cues that are considered. Since listeners integrate cues beyond F1 and F2  [@REF], this is an important gap in evaluating the plausibility of different normalization accounts as models of adaptive speech perception. All three comparisons are evaluated both separately for short and long vowels, and for the entire space of the 21 short and long vowels. This allows us to assess whether the same types of normalization perform well across the entire vowel inventory. 
...
(ref:separability) Separability indices for the Central Swedish long and short vowels separately and together in the unnormalized and normalized test data, under the assumption of F1-F2, F1-F3, F0-F3 and vowel duration as cues. Labels indicate mean separability across the five test folds. Intervals show average bootstrapped 95% confidence intervals across the test folds. Note that the ranges of the y-axes varies across plots.

```{r separability, fig.width= base.width * 4.5, fig.height = base.height * 5.5, fig.align='center', fig.cap="(ref:separability)"}
p.separability
```

## Discussion
The separability index provides a single number for the entire vowel system, which is convenient for comparability of accounts. However, this type of effect measure is limited in its exclusive focus on the within-category over between-category variability, which can become problematic for several reasons. First, in the second and third evaluations, we add dimensions to the separability index that are known to vary not as much by category as by talker (e.g., F0). We are thus potentially providing the index with, in this context, (irrelevant) information that can skew the index by adding within-category variability while leaving between-category variability (almost) unaffected. Second, most of the cue separability indices show substantial differences between normalization accounts, but the difference in absolute numbers might be small or large. For instance, separability along duration and F0 display substantial differences which are very small in absolute numbers, whereas the index for F3 show substantial differences that are large in absolute numbers. The F3 separability will therefore dominate the joint cue index, since it is a sum of the separate cue separabilities. Separability along F3 will thus weigh heavily on the overall index, regardless of its relative importance as perceptual cue. Third, given that the separability index is based on the sum of *squared variances*, a few tokens located far away from the mean (be it because of measurement error or talker-specific factors) can affect the index disproportionally. This is clearly illustrated in the density plot (Figure \@ref(fig:F3-densities), where several of the categories in unnormalized Hz and Lobanov space have a peak in one place, but a couple of points far away from the mean, resulting in overlapping densities, and low category separability. However, it is not clear that those same points will have consequences for perception. That is, the index does not directly capture whether the direction in which these extreme points deviates from the mean, is expected to cause problem for perception or not. In sum, the separability index fail to fully assess the expected consequences of normalization for perception, which is the primary interest of this study.

# Study II {#sec:studyII}
In Study II, we will assess the anticipated effects of normalization for vowel perception using models of Bayesian inference, ideal observers (IO) (see Section \@ref(sec:IOs)), to investigate how normalization accounts are predicted to affect the accuracy with which a vowel would be perceived. We ask whether normalization models differ from unnormalized models in predicting the vowel intended by the talker, and if so, which normalization model makes the strongest predictions for perception. This modelling approach avoids some of the disadvantages of the separability index. For instance, an outlier, that will weigh disproportionally heavy on a separability index, will for an IO merely be one of many points for which the IO might get a lower categorization accuracy without necessarily affecting the models' overall perception of the vowel space. Presumably much like how we imagine the human speech perception system dealing with unexpected pronunciations--outliers, in everyday life.

The IO models are assessed under the same assumptions about the relevant cues employed when evaluating the effects on category separability (see Section \@ref(sec:cues)).
...
The likelihood, $p(cues|category)$, describes the distribution of cues for each category. Here, we follow previous work and assume multivariate Gaussian distributions to describe the cue likelihood [e.g., @kronrod2016; @norris-mcqueen2008; @xie2021cognition].^[Humans have perceptual noise [@REF]. Presumably, there is now some evidence that including such perceptual noise in models provides a better fit against human behaviour [e.g., @tan2022]. Since we are interested in the relative performance of different normalization accounts, it is not apparent to us how the inclusion of noise could change the relative order of the accounts, hence, we did not include it. Additionally, studies that want to include noise would want to consider the fact that perceptual noise is represented in Mel, based on the only paper that has empirically set noise [@feldman]. It is thus also a question of technical feasibility.]<!--AP: check wording in other papers and add refs.--> In terms of representational complexity, this strikes a compromise between exemplar storage [less representationally parsimonious, @johnson1997; @pierrehumbert2001] and cue integration over multiple separate univariate Gaussians [more parsimonious, @toscano-mcmurray2010].
...
plot_io_results <- function(data) {
  data %>%
    ...
    scale_colour_manual(
      ...
      values = colors.all.procedures) +
    guides(color = "none") +
    theme(axis.text.x = ggtext::element_markdown(angle = 45, vjust = 1, hjust = 1, colour = colors.all.procedures),
        axis.title.x = element_blank())
}
...
Even though the models pattern more or less the same across vowels, there are some vowels for which models are overall good at predicting the intended category, across cue evaluations and across space sizes, e.g., [`r linguisticsdown::cond_cmpl("ɑː")`], [`r linguisticsdown::cond_cmpl("ɑ")`] and [`r linguisticsdown::cond_cmpl("ɵ")`]. Similarly, there are also some vowels for which model performance is particularly low, e.g., [`r linguisticsdown::cond_cmpl("iː")`] and [`r linguisticsdown::cond_cmpl("yː")`]. When considering only F1 and F2, the benefit of normalization for [`r linguisticsdown::cond_cmpl("iː")`] is barely visible, but as more cues are included, differences emerges between accounts. For [`r linguisticsdown::cond_cmpl("yː")`], the benefit of adding cues to models is larger than for [`r linguisticsdown::cond_cmpl("iː")`], presumably reflecting the importance of F3. However, as more cues are added, the difference between no normalization and normalization models decreases for [`r linguisticsdown::cond_cmpl("yː")`]. Which vowels will benefit from normalization will naturally vary between languages, reflecting the language-specific properties of the vowel space. For instance, [`r linguisticsdown::cond_cmpl("iː")`] is often described as more easily recognized in previous work on other languages. Its recognition accuracy is often attributed to either its absolute location in the space (peripheral vowel), or to the fact that it does not have close competitors [e.g., @Peterson1952; @Kuhl1991; @Polka2003]. Given the density of the Swedish vowel space along the height dimension, vowels such as [`r linguisticsdown::cond_cmpl("iː")`] have, on the contrary, many close competitors, which might explain the performance of the IOs.

(ref:predictions-per-vowel) Per-vowel predicted categorization accuracy of the ideal observers trained on all vowels, across the three cue spaces. Point ranges indicate the average mean accuracy and average 95% bootstrapped CI across the five folds. Chance level is indicated by grey line.
...
p.accuracy.io.per.vowel <- plot_io_results_per_vowel(
  m.io.SwehVd %>% nplyr::nest_filter(posterior_of_intended_vowel, Intended.Vowel %in% c("[ɵ]", "[a]", "[ɑː]", "[yː]", "[iː]")) %>%
...
```{r predictions-per-vowel, fig.width=base.width * 4.5, fig.height=base.height * 3.5, fig.align='center', fig.cap="(ref:predictions-per-vowel)"}
...
To further explore effects of neighbouring categories, and which categories are more easily confused by the IOs and with what, Figure \@ref(fig:confusion-matrix) displays the best and worst performing IOs' predictions about the entire Swedish vowel space, under the assumptions of F1F2, and F0-F3 and duration as cues (see \@ref(sec:confusion) for confusion matrices of all evaluations). Three general observations can be made from Figure \@ref(fig:confusion-matrix). When there is high competition from many close neighbours, models are more inclined to confuse categories with one of their neighbours. This is especially evident for the overlapping high vowels, but also for the allophones to [`r linguisticsdown::cond_cmpl("ɛ")`] and [`r linguisticsdown::cond_cmpl("ø")`]. Given that the closest neighbour is not necessarily the category's short/long counterpart, it is not primarily the case that quantity distinctions are the cause of models' confusion. As more information is given the models, confusability declines substantially. When F3 and duration are included, confusability among the high vowels is reduced, and no long-short quantity confusions remain.

In sum, these results suggest that confusability seems to primarily be driven by the number of close, or overlapping, neighbouring categories. As models presumably become more human-like, confusability decreases. The vowel-specific effects presented in this sub-section, follow the general pattern found in the overall results.
...
p.confusion.all.F1F2 <- plot_io_confusion(
  m.io.SwehVd %>% filter(IO.Normalization.Type %in% c("SyrdalGopal_Bark", "Lobanov_Hz"), Cue_space == "F1-F2", IO.Quantity == "all"),
    ...
    facet_grid(Cue_space ~ factor(IO.Normalization.Type, labels = c("SyrdalGopal (bark)", "Lobanov (Hz)"))) + theme(legend.position="none")
...
p.confusion.all.all <- plot_io_confusion(
  m.io.SwehVd %>% filter(IO.Normalization.Type %in% c("Gerstman_Hz", "CCuRE_Hz"), Cue_space == "F0-F3, duration", IO.Quantity == "all"),
                         cols = levels.vowel.IPA.swe) +
    facet_grid(Cue_space ~ factor(IO.Normalization.Type, labels = c("Gerstman (Hz)", "C-CuRE (Hz)")))

legend.confusion <- cowplot::get_legend(p.confusion.all.all +
  guides(fill = guide_legend(nrow = 1, byrow = TRUE)))
```
(ref:confusion-matrix) Confusion matrices for the worst (**left column**) and best (**right column**) performing ideal observers in predicting all 21 Swedish vowels, under different assumptions about the relevant cues: F1-F2, and F0-F3 and vowel duration. Vowel intended by talker (x-axis) is plotted against vowel selected by IO model (y-axis). Color fill indicates the posterior probability of the models predicting the intended vowel.
...
cowplot::plot_grid(legend.confusion,
  cowplot::plot_grid(p.confusion.all.F1F2, p.confusion.all.all + theme(legend.position = "none"), align = "hv", axis = "btlr", ncol = 1),
    nrow = 2, rel_heights = c(.08,1))
```

```{r}
# Plot a difference matrix, of differences in posteriors between worst and best performing models
p.diff.F1F2 <- p.confusion.all.F1F2 %+%
  (m.io.SwehVd %>%
    unnest(posterior_of_all_vowels) %>%
    pivot_longer(
    cols = levels.vowel.IPA.swe,
    ...
    aes(fill = Difference) +
    scale_fill_gradient2("Difference in\nposterior probability", limits = c(-.2,.25), breaks = seq(-.2, .25, .08)) +
    facet_wrap(~ Comparison, ncol = 1)

p.diff.all <- p.confusion.all.all %+%
  (m.io.SwehVd %>%
    unnest(posterior_of_all_vowels) %>%
    pivot_longer(
    cols = levels.vowel.IPA.swe,
...
legend.diff <- cowplot::get_legend(p.diff.all +
  guides(fill = guide_legend(nrow = 1, byrow = TRUE)))
```
(ref:diff-matrix) Difference matrices illustrating the differences in predictions between the best and the worst performing ideal observers in predicting all 21 Swedish vowels, under different assumptions about the relevant cues: F1-F2 (**panel A**), and F0-F3 and vowel duration (**panel B**). Vowel intended by talker (x-axis) is plotted against vowel selected by IO model (y-axis). Color fill indicates the difference in the posterior probability of the models predicting the intended vowel. More **purple** indicates an increase in posterior probability for the former IO over the latter IO, more **red** indicates an advantage for the latter IO over the former.

```{r diff-matrix, fig.width=base.width*3.5, fig.height=base.height*2, fig.align='center', fig.cap="(ref:diff-matrix)"}
cowplot::plot_grid(legend.diff,
  cowplot::plot_grid(p.diff.F1F2, p.diff.all + theme(legend.position = "none"), align = "hv", axis = "btlr", ncol = 2, labels = c("A", "B")),
    nrow = 2, rel_heights = c(.08,1))
```
...
As a final note on methodology, we find the 5-fold cross-validation approach used for normalization of the acoustic data adopted here, advantageous for several reasons, the two most important being that: (1) it allows us to avoid over-fitting to the sample, while also (2) providing a more realistic reflection of how parameters used for normalization are incrementally inferred from the talker's speech input. Even though many of the commonly adopted normalization accounts involve parameters that are set based on the data, previous studies have rarely, if ever, considered how these parameters might be affected by the specific dataset used for normalization.<!--AP: I'm currently checking whether this is correct.-->

## Limitations and future directions
Previous studies comparing normalization accounts often used talkers of different ages, with different language backgrounds, and perhaps most crucially, both female and male talkers [e.g., @adank2004; @carpenter1993; @escudero2007; @syrda1985]. The present study compared normalization accounts against speech from female Stockholm Swedish talkers. So, what does that mean for the generalisability of the present results? Given that gender tends to affect formants beyond talker-variability, it is likely that the inclusion of talkers of different gender would increase the lack of invariance problem. Prior to any further consideration, including male talker vowel data in the talker-independent models used in this study, would likely lower their performance. What does that mean for normalization? To the extent that normalization successfully ameliorates this issue, we expect that the benefit of normalization accounts should show more clearly, relative to unnormalized Hertz. In this sense, the present study, using only female data, is probably under-estimating the relative effect of normalization comparing to raw input. Whether the relative ordering of accounts would differ, is however unclear. To the extent that vowel-specific accounts were meant to eliminate gender differences, it is theoretically possible that the high performance of general purpose accounts (e.g., C-CuRE), might not hold when talkers of different genders are included.

The speech materials used in this study were acoustic-phonetic cues averaged across three measurement points of the vowel (at 35%, 50%, and 65% into the vowel). As such, none of the evaluations capture the dynamics of the unfolding signal. We know from previous research that formant dynamics are important for category distinctions [e.g., @Assmann2005; @hillenbrand1999; @nearey1986]. In fact, the formant trajectory across time-points is claimed to be of primary importance for some category distinctions in other varieties of Central Swedish [e.g., @kuronen2000]. A question for future research is the extent to which this claim is true, and how much better normalization models could do when normalization is applied at different time-points, to better capture the dynamics of speech perception.

Last, it is important to note that even though the normalization models in Study II performed considerably above chance in predicting the category intended by the talker, they were nevertheless far away from 100% accuracy. It is reasonable to assume that a model reflecting human speech perception could do better than 75% categorization accuracy. This 25% difference might thus indicate that there are additional, or other, mechanisms supporting robust speech perception across talkers. Even though normalization might be plausible candidate, being a pre-linguistic, automatic mechanism at the lowest processing level, perhaps it is one of several mechanisms. One of the strongest arguments *against* normalization is that, in its core assumption, it assumes that our brain normalize away talker-specific traits in the acoustic signal, implying that our speech perception system does not make use of these traits. However, we know from previous research [for reviews, see e.g., @johnson2006; @kleinschmidt-jaeger2015], that we do seem to pay attention to, and make use of, talker-specific cue characteristics when understanding talkers. This study has assumed *talker-independent* category representations, but we might very well have *talker-specific* category representations. Future studies should address this question by contrasting these hypotheses [for a review on the possible underlying mechanisms to robust inter-talker perception, and how one could computationally and experimentally approach them, see @xie2022]. More robust evidence for claims of plausibility as models for inter-talker perception, be it pre-linguistic normalization and/or talker-specific category learning, would of course be to compare models against perceptual data. This would not only allow for stronger theoretical claims, but also give us a deeper understanding of model accuracy. In order to assess what 75% model accuracy in predicting the intended vowel actually means, and to know how good our models could possibly be, we would need to relate model performance to human perceptual data. This is also our intention, moving forward.
...
```{r}
p.accuracy.io.per.vowel.long <- plot_io_results_per_vowel(
  m.io.SwehVd %>%
    filter(IO.Quantity == "long")) +
  facet_grid(Cue_space ~  factor(Intended.Vowel, levels = levels.vowel.IPA.swe))
p.accuracy.io.per.vowel.short <- plot_io_results_per_vowel(
  m.io.SwehVd %>%
    filter(IO.Quantity == "short")) +
  facet_grid(Cue_space ~  factor(Intended.Vowel, levels = levels.vowel.IPA.swe))
p.accuracy.io.per.vowel.all <- plot_io_results_per_vowel(
  m.io.SwehVd %>%
    filter(IO.Quantity == "all")) +
  facet_grid(Cue_space ~  factor(Intended.Vowel, levels = levels.vowel.IPA.swe))
```
...
```{r predictions-per-vowel-long, fig.width=base.width * 6, fig.height=base.height * 2.5, fig.align='center', fig.cap="(ref:predictions-per-vowel-long)"}
p.accuracy.io.per.vowel.long
```
\end{landscape}
...
```{r predictions-per-vowel-short, fig.width=base.width * 6, fig.height=base.height * 2.5, fig.align='center', fig.cap="(ref:predictions-per-vowel-short)"}
p.accuracy.io.per.vowel.short
```
\end{landscape}
...
```{r predictions-per-vowel-all, fig.width=base.width * 7, fig.height=base.height * 2, fig.align='center', fig.cap="(ref:predictions-per-vowel-all)"}
p.accuracy.io.per.vowel.all
```
...
p.confusion.long.F1F2 <- plot_io_confusion(
  m.io.SwehVd %>% filter(IO.Normalization.Type %in% c("Miller_log", "Lobanov_Hz"), Cue_space == "F1-F2", IO.Quantity == "long"),
    ...
    facet_grid(Cue_space ~ factor(IO.Normalization.Type, labels = c("Miller (log)", "Lobanov (Hz)"))) + theme(legend.position="none")
...
p.confusion.long.all <- plot_io_confusion(
  m.io.SwehVd %>% filter(IO.Normalization.Type %in% c("Gerstman_Hz", "Lobanov_Hz"), Cue_space == "F0-F3, duration", IO.Quantity == "long"),
                         cols = levels.vowel.IPA.swe.long) +
    facet_grid(Cue_space ~ factor(IO.Normalization.Type, labels = c("Gerstman (Hz)", "Lobanov (Hz)"))) + theme(legend.position="none")
...
p.confusion.short.F1F2 <- plot_io_confusion(
  m.io.SwehVd %>% filter(IO.Normalization.Type %in% c("Miller_log", "CCuRE_Mel"), Cue_space == "F1-F2", IO.Quantity == "short"),
    ...
    facet_grid(Cue_space ~ factor(IO.Normalization.Type, labels = c("Miller (log)", "C-CuRE (Mel)"))) + theme(legend.position="none")
...
```{r confusion-matrix-allVowels, fig.width=base.width*3, fig.height=base.height*4.5, fig.align='center', fig.cap="(ref:confusion-matrix-allVowels)"}
cowplot::plot_grid(legend.confusion,
