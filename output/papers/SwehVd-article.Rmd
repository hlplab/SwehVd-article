...
```{r illustrate-normalization, fig.width=base.width*2.5, fig.height=base.height*3.5+.5, out.width='70%', fig.align='center', fig.cap="(ref:illustrate-normalization)"}
cowplot::plot_grid(
  legend,
  cowplot::plot_grid(
...
The likelihood, $p(cues|category)$, describes the distribution of cues for each category. Here, we follow previous work and assume multivariate Gaussian distributions to describe the cue likelihood [e.g., @kronrod2016; @norris-mcqueen2008; @xie2021cognition].^[Humans have perceptual noise. Presumably, there is now some evidence that including such perceptual noise in models provides a better fit against human behaviour [e.g., @tan2022]. Since we are interested in the relative performance of different normalization accounts, it is not apparent to us how the inclusion of noise could change the relative order of the accounts, hence, we did not include it. Additionally, studies that want to include noise would want to consider the fact that perceptual noise, based on the only paper that has set noise [@feldman], is represented in Mel. It is thus also a question of technical feasibility.]<!--AP: check wording in other papers and add refs.--> In terms of representational complexity, this strikes a compromise between exemplar storage [less representationally parsimonious, @johnson1997; @pierrehumbert2001] and cue integration over multiple separate univariate Gaussians [more parsimonious, @toscano-mcmurray2010].
...
# }
# 
# m.io.SwehVd.for.plotting <- m.io.SwehVd %>%
#   # Rename IO.Normalization.Type for plotting
#   mutate(
#     IO.Normalization.Type =
#       factor(
...
#   m.io.SwehVd.for.plotting %>%
#     filter(Cue_space == "F1-F2", IO.Quantity %in% c("long", "short"), Intended.Vowel %in% c("[ɵ]", "[a]", "[ɑː]", "[yː]", "[iː]"))) +
#   facet_grid(Cue_space ~  factor(Intended.Vowel, levels = levels.vowel.IPA.swe))
```

\begin{landscape}

```{r predictions-per-vowel, fig.width=base.width * 3.5, fig.height=base.height * 1.5, fig.align='center', fig.cap="(ref:predictions-per-vowel)"}
...
p.confusion.F1F2 <- plot_io_confusion(
  m.io.SwehVd %>% filter(IO.Normalization.Type %in% c("Miller_log", "Lobanov_Hz"), Cue_space == "F1-F2", IO.Quantity == "all"),
...
p.confusion.all <- plot_io_confusion(
...
legend.confusion <- cowplot::get_legend(p.confusion.all +
...
cowplot::plot_grid(legend.confusion,
  cowplot::plot_grid(p.confusion.F1F2, p.confusion.all + theme(legend.position = "none"), align = "hv", axis = "btlr", labels = c("A", "B"), ncol = 1),
    nrow = 2, rel_heights = c(.08,1))
```

```{r}
# Generate dataframe with difference in posteriors between talker-independent and talker-specific models
# m.io.SwehVd.diff <-
#   m.io.SwehVd.for.plotting %>%
#     filter(IO.Normalization.Type %in% c("Miller_log", "Lobanov_Hz"), Cue_space == "F1-F2", IO.Quantity == "all") %>%
#     pivot_longer(
#       cols = cols,
#       values_to = "Posterior",
#       names_to = "IO.Vowel") %>%
#     filter(IO.Vowel == Vowel) %>%
#   select(c(IO.Quantity, Intended.Vowel, IO.Normalization.Type, IO.Vowel, Posterior, F1, F2)) %>%
#   pivot_wider(
#     names_from = c(IO.Quantity, IO.Normalization.Type),
#     values_from = Posterior) %>%
#   mutate(
#     miller_IO_Advantage.Probability = Miller - Lobanov,
#     lobanov_IO_Advantage.Probability = Lobanov - Miller)
...
#       values_from = Posterior) %>%
#     mutate(
#       "Difference Miller-Lobanov" = `Miller (log)` - `Lobanov (Hz)`,
#       "Difference Gerstman-C-CuRE Hz" = `Gerstman (Hz)` - `C-CuRE (Hz)`) %>%
#   #   na.omit() %>%
...
#     m.io.SwehVd %>% filter(IO.Normalization.Type %in% c("Miller_log", "Lobanov_Hz"), Cue_space == "F1-F3", IO.Quantity == "all"), cols = levels.vowel.IPA.swe) + facet_grid(Cue_space ~ factor(IO.Normalization.Type, labels = c("Miller (log)", "Lobanov (Hz)"))) + theme(legend.position="none")
...
```{r confusion-matrix-long, fig.width=base.width*3, fig.height=base.height*4, fig.align='center', fig.cap="(ref:confusion-matrix-long)"}
cowplot::plot_grid(legend.confusion,
  cowplot::plot_grid(p.confusion.long.F1F2, p.confusion.long.F1F3, p.confusion.long.all + theme(legend.position = "none"), align = "hv", axis = "btlr", labels = c("A", "B", "C"), ncol = 1),
    nrow = 2, rel_heights = c(.08,1))
```
...
```{r confusion-matrix-short, fig.width=base.width*3, fig.height=base.height*4, fig.align='center', fig.cap="(ref:confusion-matrix-short)"}
cowplot::plot_grid(legend.confusion,
  cowplot::plot_grid(p.confusion.short.F1F2, p.confusion.short.F1F3, p.confusion.short.all + theme(legend.position = "none"), align = "hv", axis = "btlr", labels = c("A", "B", "C"), ncol = 1),
    nrow = 2, rel_heights = c(.08,1))
```
...
# cowplot::plot_grid(legend.confusion,
#   cowplot::plot_grid(p.confusion.all.F1F2, p.confusion.all.F1F3, p.confusion.all.all + theme(legend.position = "none"), align = "hv", axis = "btlr", labels = c("A", "B", "C"), ncol = 1),
#     nrow = 2, rel_heights = c(.08,1))
```
