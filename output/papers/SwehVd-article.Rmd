...
```{r illustrate-normalization, fig.width=base.width*2+.5, fig.height=base.height*2, out.width='60%', fig.align='center', fig.cap="(ref:illustrate-normalization)"}
cowplot::plot_grid(
  legend,
  cowplot::plot_grid(
...
```{r hedd, fig.width=base.width*1.5, fig.height=base.height*1.5, out.width='40%', fig.align='center', fig.cap="(ref:hedd)"}
d.SwehVd %>% 
...
```{r swe-vowels, fig.width=base.width*2, fig.height=base.height*2.5, out.width='50%', fig.align='center', fig.cap="(ref:swe-vowels)"}
d.SwehVd %>% 
  filter(Transcribed_vowel == "targeted", Word != "hädd") %>%
  # Get the geometric mean F1-F2 across the five time points
...
```{r swe-vowels-all-cues, fig.width=base.width*3.5, fig.height=base.height*3.5+.5, fig.cap="(ref:swe-vowels-all-cues)"}
p.matrix.cues <- 
  d.SwehVd %>% 
  filter(Transcribed_vowel == "targeted", Word != "hädd") %>%
...
```{r swe-vowels-normalized-long, fig.width=base.width * 3.5, fig.height=base.height * 2.5, fig.align='center', fig.cap="(ref:swe-vowels-normalized-long)"}
p.vowels.norm.long
```
...
\begin{landscape}
```{r swe-vowels-normalized-short, fig.width=base.width * 3.5, fig.height=base.height * 2.5, fig.align='center', fig.cap="(ref:swe-vowels-normalized-short)"}
p.vowels.norm.short
```
...
In order to make the separability index more transparent, we begin by presenting the separability index for all five cues separately, for the long and short vowels, or all vowels together (Figure \@ref(fig:separability-by-cue)). For F1 (**Panel A**, Figure \@ref(fig:separability-by-cue)), we see a clear difference between centering and not centering accounts, but also that scaling, such as Lobanov, further improves category separability along F1. The difference between accounts emerge differently along F2 (**Panel B**, Figure \@ref(fig:separability-by-cue)), where it appears as if the variability within each group of accounts is larger than between, for instance, scaling and the others. The large difference between centering and non centering accounts for F1, is considerably smaller for F2. It seems as if scaling accounts efficiently increase separability along F1, but less so along F2. Nearey2 is now performing better than Nearey1, and Gerstman still worse than Lobanov. <!-- AP: should the large CIs be commented?--> For category separability along F3 (**Panel C**, Figure \@ref(fig:separability-by-cue)), the results change drastically compared to F1 and F2. This is presumably the first finding that should give pause for thought. There is a large difference between the intrinsic accounts, that perform really well in separating categories along F3, and all other accounts, which is surprising. Previous work [@REFNORM] has pointed out that one of the main downsides of intrinsic approaches, such as Syrdal & Gopal, is that they use a single estimate when normalizing the formants on a vowel by only taking into account other formants on that same segment. This would presumably be particularly difficult for F3, as it is often more difficult to reliably estimate, leading to measurement noise and more room for error, which could get even worse in languages with rhorotized vowels. This might very well be the case here, however, in a language as Swedish where F3 carries important information about vowel quality, these approaches still come out as a very clear net-advantage for F3. To further illustrate this, we plot the F3 densities of each of the categories in the five test folds (Figure \@ref(fig:F3-densities)), in unnormalized Hz, Syrdal & Gopal, and Lobanov. If we look at the five test folds, we see that there is some variability across folds, but we also see that the separation and ordering is the same across folds, and that there is a huge advantage in F3 for the intrinsic accounts. <!--AP: Double-check that it’s doing the same thing as Miller.--> <!-- AP: Does it seem to carry information about lip rounding? Check the vowels.--> For the unnormalized and Lobanov normalized categories, we see that density peaks are separated, but that there is a lot of overlap. Critically, almost all of the category F3 densities have points that are far away from the mean. It is possible that those points might be outliers caused by measurement error, that we did not manage to capture. However, we used the same procedure as for F1 and F2, and the densities do overlap. These large differences between instrinc accounts and the others, will presumably have an effect on when assessing the overall separability under assumption of the relevant cues to vowel identity. Before we assess the joint separability along several of the cues, we come back to Figure \@ref(fig:separability-by-cue) and the separability along F0 (**Panel D**). <!-- AP: develop this part.-->
Finally, **Panel E**, Figure \@ref(fig:separability-by-cue), indicate separability along Duration. Clearly, all of the C-CuRE accounts group together, given that these are the only accounts that permits other cues than formants. We had the choice to C-CuRE duration for all other accounts as well, but since the other approaches does not account for duration, we believed it to be more informative to show how C-CuRE performs in comparison to other approaches that does not account for duration.

Figure \@ref(fig:separability-by-cue) clearly indicates that the category separability is affected along the cues in different ways. Some of these results are expected: for instance, it is expected that F0 will not support category separability much, since it is known to differ between talkers, but not between categories. For duration, it is expected that separability will improve for C-CuRE, as all other accounts do not transform the input but rather use the raw data (in ms). Given that categories emerge more clearly in F1-F2 space, it is also expected that separability will be higher in these cues. More surprising, however, is the result for F3, that clearly affects the normalization accounts' performance unexpectedly. These differences between the different cue indices, will of course transfer to when we calculate the separability index under the different assumption about the relevant cues.
...
p.separability.F3_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "F3")) + facet_grid(~ factor(Quantity, levels = c("long vowels", "short vowels", "all vowels")))
p.separability.F2_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "F2")) + facet_grid(~ factor(Quantity, levels = c("long vowels", "short vowels", "all vowels")))
p.separability.F1_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "F1")) + facet_grid(~ factor(Quantity, levels = c("long vowels", "short vowels", "all vowels")))
p.separability.F0_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "F0")) + facet_grid(~ factor(Quantity, levels = c("long vowels", "short vowels", "all vowels")))
p.separability.Duration_by_quantity <- plot_separability(d.SwehVd.forStudy.SI.long %>%
  filter(Cues == "Duration")) + facet_grid(~ factor(Quantity, levels = c("long vowels", "short vowels", "all vowels")))
```
...
```{r separability-by-cue, fig.width= base.width * 3.5, fig.height = base.height * 5, fig.align='center', fig.cap="(ref:separability-by-cue)"}
cowplot::plot_grid(p.separability.F1_by_quantity + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()), 
                   p.separability.F2_by_quantity + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()), 
                   p.separability.F3_by_quantity + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()), 
                   p.separability.F0_by_quantity + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()), 
                   p.separability.Duration_by_quantity, ncol = 1, align = "hv", axis = "btlr", labels = c("A", "B", "C", "D", "E"), scale = 0.9)
```
(ref:F3-densities) Category F3 densities for the Central Swedish long (**Panel A**) and short (**Panel B**) vowels in unnormalized Hz and normalized test data (Syrdal & Gopal normalization, and Lobanov normalization). Number in label indicates test-fold (1-5).

```{r F3-densities, fig.width= base.width * 3.5, fig.height = base.height * 3.5, fig.align='center', fig.cap="(ref:F3-densities)"}
...
p.F3.test.long <- d.SwehVd.forStudy.long %>% 
  ...
  facet_wrap(Normalization.Type ~ crossvalidation_group, scales = "free", ncol = 5, labeller = label_wrap_gen(multi_line=FALSE))
...
cowplot::plot_grid(p.F3.test.long, p.F3.test.short, labels = c("A", "B"), ncol = 1)
```

\begin{landscape}

```{r fig.width= base.width * 4, fig.height = base.height * 2.5, out.width='100%', fig.align='center'}
d.SwehVd.forStudy.long %>% 
  filter(Quantity == "long", fold_type == "test", Normalization.Type %in% c("no normalization (Hz)", "SyrdalGopal (Bark)", "Lobanov (Hz)")) %>%
  ggplot(
...
d.SwehVd.forStudy.long %>% 
  ...
  ggplot(
    aes(F3, 
        label = category,
        color = category)) +
  geomtextpath::geom_textdensity(mapping = aes(linetype = Quantity),
                                  alpha = .3, position = "identity") +
  scale_colour_manual(name = "category", values = colors.vowel.swe, aesthetics = c("color", "fill")) + 
...
<!--AP: rewrite and double-check results.-->
Figure \@ref(fig:separability) shows the separability index for the normalization accounts investigated, under the different assumptions about the relevant cues (Panel A-C) and depending on whether separability was calculated separately for the long and short vowels, or together. The first evaluation, follow most previous research in assessing category separability along F1 and F2 (**Panel A**). We see here that accounts that center against the talker's overall formant mean, such as C-CuRE [@mcmurray-jongman2011] or @nearey1978 (in blue), are among the highest performing normalization accounts, independent of vowel space size. Also, no matter the assumed perceptual scale, centering always improves category separability (compare transformation accounts, in grey, normalized with C-CuRE, in blue). Furthermore, the performance of what has been considered high-performing accounts in previous studies, such as @gerstman1968 and @lobanov1971 (in pink), varies as more cues are considered. Considering F1-F2, the results overall replicate previous findings [e.g., @disner1980; @fabricius2009; @Flynn2011]. 

For the SI, if you have one token that is far away, based on squares, sum of squares variances, it can disproportionally much affect things.

For a language like Swedish, where F3 carry information to vowel identity, intrinsic accounts such as SyrdalGopal and Miller that appear to do very well at separating categories along F3 and successfully distinguish catgories, seem to be particularly helpful, in comparison to other accounts. It seems to help in the dense parts of the space, or to help distinguish the denser parts from the other parts.

When including F3, and further vowel duration, the same accounts perform considerably worse, with a separability index even lower than unnormalized cues. This does not replicate previous findings where F3 has been included [e.g., @adank2004]. However, these studies used a different effect measure. It might be the case that the inclusion of additional cues that display large between-talker variability but rather small between-category variance, such as F0, generates lower separability indices. (3) Some transformations to perceptual scales compare quite well with other accounts across evaluations, e.g. Bark. They do not outperform normalization accounts, but they do rank as good as some accounts, e.g. @miller1989c This replicates previous findings[@REFS]. (4) The separability index varies between long and short vowels for some normalization accounts. In general, category separability is higher for the short vowels across evaluations. Given that the short vowels form a more centralized and dense space, this result might appear puzzling. However, as indicated in Figure \@ref(fig:swe-vowels), the long vowels are more overlapping than the short, especially in F1-F2 space. Overall, the separability indices more or less aligns with the results from the visual inspection of scatter reduction introduced by applying normalization accounts (see Figures \@ref(fig:swe-vowels-normalized-long) and \@ref(fig:swe-vowels-normalized-short)). For instance, short vowels are visually more separated under C-CuRE comparing to the long vowels. (5) As more cues are included, the separability index goes down. The pattern with higher indices for the short vowels persist across evaluations. (6) (5) is also true when including vowel duration as additional cue. (7) Evaluating effects on category separability for all vowels together does not seem to make much difference. <!-- relate back to matrix plot?!-->Comparing to previous work, assessing effect of normalization on category variability using F-statistics (e.g., @labov2010), we note that the same top performing accounts for F1 and F2 are <!-- fill in here-->. However, since vowel duration has not been evaluated as cue...
<!-- AP: reorganize the results section. Both F0 and F3 are know to vary not as much by category as by talker - REF Kuronen - for F3 across vowels. The high performance of SyrdalGopal and Miller seems to be driven by their ability to increase category separability in F3. Neither of the previous studies that assessed efficacy by reduction in variance, and found Lobanov among the top two, looked at other cues besides F1 and F2: @fabricius2009 and @Flynn2011 assessed by variance reduction, Labov at F-stats. Furthermore, Labov didn't even include Lobanov or any other scaling procedure.-->
...
<!-- AP: finish with this.-->
The separability index gives us a single number for the entire vowel system, which is convenient for comparability, but it does not capture everything. The only thing the measure cares about is the within-category over between-category variability. If the within-category variability increases, for instance, because of some points far away from the mean, the measure does not directly capture whether the direction in which these extreme points deviates from the mean is expected to cause problem for perception or not. Since it is about squared errors, points that are far away from the mean will weigh potentially unproportionally heavily. This becomes evident in the density plots, where many of the categories have a peak in one place, but points far away from the mean. Whenever that is the case the separability index will punush for that, but it is not clear that those points will have consequences for perception. Next, we will in a coherent way assess this for perception.
...
```{r separability, fig.width= base.width * 3.5, fig.height = base.height * 5, fig.align='center', fig.cap="(ref:separability)"}
cowplot::plot_grid(
...
In Study I, we evaluated the extent to which normalization removes category overlap due to inter-talker variability, using visual inspection and a separability index. However, as we saw in Section \@ref(sec:normVariability), this measure comes with certain disadvantages. The separability index is summarized down to one number for the entire vowel system. If there is one point that is far away from the mean, be it by an unusual talker, or measurement error, this points will inevitably weigh heavily on the index. In Study II, we will assess the anticipated effects of normalization for vowel perception, using a modelling approach that allows us to look at the entire vowel system in a different way. Using ideal observer models (see Section \@ref(sec:IOs)), we can <!-- AP: develop this part--> . Furthermore, an outlier, that will weigh heavily on a separability index, will for an ideal observer only be one of many points for which you might get a lower categorization accuracy. In Study II, we investigate how normalization accounts are predicted to affect the accuracy with which a vowel would be perceived. We ask whether normalization models differ from unnormalized models in predicting vowel categorization, and if so, which normalization model makes the strongest predictions for perception. Models are assessed under the same assumptions about the relevant cues employed when evaluating the effects on category separability (see Section \@ref(sec:cues)).
...
```{r confusion-matrix, fig.width=base.width*3.5, fig.height=base.height*3.5+.5, out.height='60%', fig.align='center', fig.cap="(ref:confusion-matrix)"}
...
This paper has investigated the effect of low-level pre-linguistic normalization on Stockholm Swedish vowels, by assessing different accounts' effects on category separability, and their predicted consequences for perception. Evaluating 15 different normalization accounts against the dense vowel space of Swedish, has allowed us to assess whether the same normalization accounts that work well for vowel systems with fewer categories, work equally well for more complex systems. Investigating a language with a systematic quantity distinction has furthermore allowed for a bridging of the literature on vowel-specific accounts and on general normalization accounts [such as C-CuRE, @mcmurray-jongman2011], and for expanding on previous research that primarily has focused on the first two (or three) formants. 

The speech materials used was a subset of the SwehVd database, a new phonetically annotated database of Stockholm Swedish vowels that was presented and released in this paper. The following methods were used for evaluation. In Study I, we compared the reduction of within-category variability relative to between-category variability by calculating a separability index (i.e., an F-statistics). This methodology facilitates comparison to previous studies, that has most often assessed the efficacy of normalization accounts by means of increase in category overlap or category separability, or decrease in category variability [@disner1980; @fabricius2009; @Flynn2011; @hindle1978; @labov2010]. This index, however, <!--AP: comment here on the SI-->

In Study II, we assessed the predicted consequences for perception by training ideal observer models on the acoustic data transformed into different normalization spaces, under different assumptions about the relevant cues. Previous studies have primarily used models of linear discriminant analysis [e.g., @adank2004; @syrda1985], fuzzy ARTMAP and K-nearest neighbour [e.g., @carpenter1993], or constraint ranking grammars [e.g., @escudero2007]. We find several advantages with the modelling approach adopted here. First, IO models have 0 degrees of freedom in the link from production to perception, thus removing the researchers' degrees of freedom in the evaluation of normalization accounts. Second, representing the cue likelihood for each category as multivariate Gaussian distributions [e.g., @kronrod2016; @norris-mcqueen2008; @xie2021cognition], strikes a middle ground between less parsimonious models such as exemplar models [e.g., @johnson1997; @pierrehumbert2001], and more parsimonious, such as models of cue integration over multiple separate univariate Gaussians [e.g., @toscano-mcmurray2010]. And finally, we find that the 5-fold cross-validation approach more realistically reflects how parameters used for normalization is incrementally inferred from the talker's speech input.^[We would like to remind readers that all of the assumptions in the model (e.g., uniform priors, models of linguistic representations, the normalization accounts selected) can be revisited and altered by the reader, as this paper is written in R markdown and all data and code is provided in the SI/OFS.]

The results from Study I showed that the same type of accounts that have been proven successful in previous evaluations on other languages, are also successful in improving category separability on Swedish vowels. The increase in category separability for the best performing normalization accounts, seems to be equally large as those reported by @disner1980 for the other languages in her cross-linguistic study.<!-- Reduction in variance for Swe = 41%, for the other languages 61-71%--> Furthermore, here, simple centering performs as good, or even better, for Swedish as it did for the other languages in her study. In conclusion, it might well be the case that the (somewhat) diverging results for Swedish in @disner1980's study, was not caused by properties inherent to Swedish, but more likely an effect of the nature of the dataset used. Here, we compare model predictions against the talkers' intended category, based on recordings of hVd words, which is presumably more comparable to the datasets of the other Germanic languages, besides Swedish, used in @disner1980.<!-- AP: or male vs female--> However, ....<!-- AP: check results om Labov, Barreda/Nearey, that also used Fstats + this cannot be finalized until we fully understand what is going on with the sep index-->. 

Study II indicated that Lobanov and centering approaches, vowel-specific or general such as Nearey or C-CuRE, are the top-performing accounts, which generally replicates previous studies comparing predictions against production data [e.g., @adank2004; @barreda2018a; @carpenter1993; @escudero2007; @syrda1985]. As more cues are included, presumably making models more human-like, the relative advantage of Lobanov normalization seems to decrease, possible indicating that simple centering might make up a plausible candidate for a mechanism supporting robust inter-talker perception. More robust evidence for such a claim, would of course be to compare models against perceptual data. This is also our intention, moving forward... 
...
<!-- AP: points to be integrated-->
  * Its plausibility as a model for robust inter-talker perception...
  * Far away from 100%, normalization and/or additional mechanisms plausible to account for robust inter-talker perception, such as talker-specific learning, etc. This study has assumed talker-independent category representations...
  * More future: train IOs on the onfolding signal, so at all timepoints?
  * keep in mind, some talkers have less data (very few talkers though, no more 5 vs 10 though)
  * keep in mind, all of them are female. As a theory of how physiological differences are removed, if nothing, these results might under-estimate normalization as mechanism for supporting inter-talker perception??? 

<!-- AP: B&N does this whole analysis of suitability of approaches as models of human cognition, where these measures are included, but is it relevant here?: calculating amount of error in normalization parameter estimates [e.g., @barreda2018a]-->
