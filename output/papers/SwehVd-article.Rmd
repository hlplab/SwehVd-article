...
# To Do
  ...
  * Add REFS
    * check author names in bib-file
    * add @nearey1989 to the table
...
In order to assess the effects of normalization accounts on category separability, we calculate a separability index under different assumptions about the relevant cues and the size of the vowel space (the long and short vowels separately, or the entire space). Before we evaluate how category separability is affected by normalization in F1-F2, F0-F3, and F0-F3 and duration space, we look at how the normalization accounts affect the separability of vowels along each cue separately (Figure \@ref(fig:separability-by-cue)). As we show below, this is helpful in understanding the subsequently presented results for combinations of cues.
...
p.separability <- plot_separability(d.SwehVd.forStudy.SI.long %>%
                                             filter(Cues %in% c("F1F2", "F1F3", "F0F3", "all"))) +
  facet_grid(factor(Cues, levels = c("F1F2", "F1F3", "F0F3", "all"), labels = c("F1-F2", "F1-F3", "F0-F3", "F0-F3, duration")) ~ factor(Quantity, levels = c("long vowels", "short vowels", "all vowels")), scales = "free_y")
```
...
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{5cm}>{\raggedright\arraybackslash}p{5cm}>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{2cm}}
\toprule
Language(s) investigated & Article & Speech materials & Normalization accounts & Approach & Accuracy assessed & Best two performing\\
\midrule
 & Barreda \& Nearey, 2018 & 120,000 simulated languages (of 5 or 9 vowels) modeled on Hillenbrand et al.'s (1995) data (138 female/male child/adult talkers * 12 vowels) & Nearey2, Lobanov, log-mean in linear regression framework & regression, F statistics &  & log-mean in linear regression framework (1), Nearey2 (2)\\
\cmidrule{2-5}
\cmidrule{7-7}
...
\cmidrule{2-5}
\cmidrule{7-7}
 & McMurray, Cole \& Munson, 2011 & Cole et al. (2010) database, 10 female/male talkers (1 token * 2 target vowels * 4 context vowels * 6 consonants) &  &  & \multirow{-5}{2cm}{\raggedright\arraybackslash against intended category} & \\
\cmidrule{2-3}
\cmidrule{6-6}
 & McMurray \& Jongman, 2016 & Jongman et al. (2000) database, 10 female/male talkers (1 token * 4 vowels * 8 fricatives) & \multirow{-2}{5cm}{\raggedright\arraybackslash C-CuRE} & \multirow{-2}{2cm}{\raggedright\arraybackslash regression} & against perceived category & \multirow{-2}{2cm}{\raggedright\arraybackslash C-CuRE (1)}\\
\cmidrule{2-7}
 & Richter, Feldman, Salgado \& Jansen, 2017 & Clopper \& Pisoni's (2006) NSP vowel corpus, 60 female/male talkers, 6 varieties (5 tokens * 10 vowels) & Vocal Tract Length Normalization, Lobanov & discrimination model likelihoods & against perceived category (Feldman et al., 2009) & Vocal Tract Length Normalization (1), Lobanov (2)\\
\cmidrule{2-7}
\multirow{-8}{2cm}{\raggedright\arraybackslash US English} & Syrdal, 1985 & Peterson \& Barney's (1952) database, 75 female/male child/adult talkers (2 tokens * 10 vowels); Texas Instruments database, 154 female/male child/adult talkers (1 token * 10 vowels) & log-transformation, Bark, Syrdal's bark-difference model, Miller (2 accounts), Nearey1, Nearey2, Gerstman & linear discriminant analysis &  & Syrdal \& Gopal (1), Nearey1/Nearey2 (2)\\
\cmidrule{1-5}
\cmidrule{7-7}
...
```{r}
#Remove temp file
rm(d.max.acc.toINTEND, d.max.acc.toINTEND.diff)
...
p.accuracy.io.per.vowel.all.1 <- plot_io_results_per_vowel(
  m.io.SwehVd %>%
    filter(IO.Quantity == "all") %>%
    nplyr::nest_filter(posterior_of_intended_vowel, Intended.Vowel %in% c("[iː]", "[ɪ]", "[yː]", "[ʏ]", "[ʉː]", "[ɵ]", "[eː]", "[ɛ]", "[ɛː]"))) +
  facet_grid(Cue_space ~  factor(Intended.Vowel, levels = levels.vowel.IPA.swe))
p.accuracy.io.per.vowel.all.2 <- plot_io_results_per_vowel(
  m.io.SwehVd %>%
    filter(IO.Quantity == "all") %>%
    nplyr::nest_filter(posterior_of_intended_vowel, Intended.Vowel %in% c("[æː]", "[æ]", "[øː]", "[ø]", "[œː]", "[œ]", "[ɑː]", "[a]", "[oː]", "[ɔ]", "[uː]", "[ʊ]"))) +
  facet_grid(Cue_space ~  factor(Intended.Vowel, levels = levels.vowel.IPA.swe))
```
...
```{r predictions-per-vowel-all, fig.width=base.width * 7, fig.height=base.height * 2.5, fig.align='center', fig.cap="(ref:predictions-per-vowel-all)"}
p.accuracy.io.per.vowel.all.1
```
\end{landscape}
...
```{r predictions-per-vowel-all-2, fig.width=base.width * 7, fig.height=base.height * 2.5, out.width='110%', fig.align='center', fig.cap="(ref:predictions-per-vowel-all-2)"}
p.accuracy.io.per.vowel.all.2
```
\end{landscape}
...
cowplot::plot_grid(legend.confusion,
  cowplot::plot_grid(p.confusion.all.F1F2, p.confusion.all.F1F3, p.confusion.all.all + theme(legend.position = "none"), align = "hv", axis = "btlr", ncol = 1),
    nrow = 2, rel_heights = c(.08,1)) 
```
<!--AP: add text on difference matrix here-->
