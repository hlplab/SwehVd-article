...
d.data.A <- m.io.SwehVd.F1F2 %>%
  ...
  mutate_at(
      vars(contains("Sigma")),
      funs(map(. , .f = function(x)  {x * .6}))) %>%
  group_by(category) %>%
  mutate(
    samples = pmap(list(60, mu, Sigma), rmvnorm)) %>%
  unnest(samples) %>%
  mutate(
    F1 = samples[,1],
    F2 = samples[,2],
...
p.marginal.joint.1cat <- d.data.A %>%
  ...
  geom_point(
    aes(x=quantile(d.data.A$F2, probs = .95),
        y=quantile(d.data.A$F1, probs = .05)),
    color = "black",
    size = 1) +
    guides(color = "none") +
  geom_point(
    aes(x=quantile(d.data.A$F2, probs = .95),
        y=quantile(d.data.A$F1, probs = .95)),
    color = "black",
    size = 1) +
    guides(color = "none") +
  geom_rug(
    mapping = aes(
      x=quantile(d.data.A$F2, probs = .95),
        y=quantile(d.data.A$F1, probs = .05)),
    color = "black",
    sides = "tr",
...
xdens <- cowplot::axis_canvas(p.marginal.joint.1cat, axis = "x") +
  #stat_function(fun = dnorm(mean = d.data.A$mu[[1]], sd = d.data.A$Sigma[[1]]), n = n) +
...
ydens <- cowplot::axis_canvas(p.marginal.joint.1cat, axis = "y", coord_flip = TRUE) +
  geom_density(data = d.data.A %>% filter(category == "[ɑː]"), aes(x = F1, fill = category),
               alpha = 0.7, size = 0.2) +
  coord_flip() + 
  scale_fill_manual(values = colors.vowel.swe) +
  scale_x_reverse()
p1 <- cowplot::insert_xaxis_grob(p.marginal.joint.1cat, xdens, grid::unit(.2, "null"), position = "top")
