...
In order to assess the affects of normalization accounts on category separability, we calculate a separability index under different assumptions about the relevant cues (F1-F2, F1-F3, F0-F3 and vowel duration) and size of the vowel space (the long and short vowels separately, or the entire space). First, we begin by presenting the indices for all five cues separately, to add some transparency to the indices and make some general points (Figure \@ref(fig:separability-by-cue)). 

For F1 (**Panel A**, Figure \@ref(fig:separability-by-cue)), we see a clear difference between centering and not centering accounts. We also note that scaling accounts, such as Lobanov, further improves category separability along F1, especially for the long vowels and the entire vowel space. The accounts are positioned differently along F2 though (**Panel B**, Figure \@ref(fig:separability-by-cue)). It appears as if the variability within each group of accounts is larger than between, for instance, scaling and the others. The rather large difference between centering and non centering accounts along F1, is considerably smaller for F2. Even though scaling accounts efficiently increase category separability along F1, they are less efficient along F2. <!-- AP: should the large CIs be commented?--> 

The most drastic change is in category separability along F3 (**Panel C**, Figure \@ref(fig:separability-by-cue)), compared to F1 and F2. There is a large difference between the intrinsic accounts, that perform really well in separating categories along F3, and all other accounts, which is surprising. Previous work [@REFNORM] has pointed out that one of the main downsides of intrinsic approaches, such as @Syrdal1986, is that they are sensitive to measurement error as they use a single estimate for normalization, by only taking other formants on that same segment into account when normalizing formants, instead of the overall formant mean. F3 could presumably be particularly affected, as it is claimed to be more difficult to reliably estimate than other formants, leading to measurement noise and more room for error. This might very well be the cause of these results, however, in a language as Swedish where F3 carries important information about vowel quality, these approaches still come out with a very clear net-advantage for F3. To further illustrate this, we plot the F3 densities of each of the categories in the five test folds (Figure \@ref(fig:F3-densities)), in unnormalized Hz, @Syrdal1986, and @lobanov1971. Figure \@ref(fig:F3-densities) indicates that even though there is some variability, the separation and ordering is the same across folds, and we do see a huge advantage in F3 for the intrinsic account. Comparing to unnormalized Hz and @lobanov1971, @Syrdal1986 seems to be particularly successful with regard to some of the overlapping long high vowels, where F3 is supposedly carrying information about lip rounding to aid in category distinction. @Syrdal1986 separates two clusters among the high vowels, one consisting of the back vowels `r linguisticsdown::cond_cmpl("[oː]")` and `r linguisticsdown::cond_cmpl("[uː]")`, and the other one of the front `r linguisticsdown::cond_cmpl("[eː]")`, `r linguisticsdown::cond_cmpl("[iː]")`, `r linguisticsdown::cond_cmpl("[yː]")` and mid `r linguisticsdown::cond_cmpl("[ʉː]")`. Among these two clusters, @Syrdal1986 seems to be slightly better at separating the peaks of `r linguisticsdown::cond_cmpl("[iː]")` and `r linguisticsdown::cond_cmpl("[yː]")`. For the unnormalized and Lobanov normalized categories, we see that density peaks are separated, but that there is a lot of overlap. Critically, almost all of the category F3 densities have points that are far away from the mean. It is possible that those points might be outliers caused by measurement error, that we did not manage to capture. However, we used the same procedure as for F1 and F2, and the densities do overlap. These large differences between instrinsic accounts and the others, will presumably have an effect on when assessing the overall separability under assumption of the relevant joint cues to vowel identity.

Before assessing the joint separability along multiple cues, we make some final comments on the separability index for F0 and duration (Figure \@ref(fig:separability-by-cue)). For separability along F0 (**Panel D**, Figure \@ref(fig:separability-by-cue)), the accounts pattern similarly to F1. Overall, accounts do not seem to increase category separability along F0 to any substantial degree, which is expected given that F0 is known to differ primarily between talkers, rather than between vowel categories. Finally, **Panel E**, Figure \@ref(fig:separability-by-cue), indicate separability along duration. Clearly, all of the C-CuRE accounts group together, given that these are the only accounts that permit other cues than formants. Figure \@ref(fig:duration-densities) further illustrates the duration densities in unnormalized Hz and C-CuRE (Hz), for each of the five test folds. In comparison to Figure \@ref(fig:F3-densities), Figure \@ref(fig:duration-densities) indicates a more clear separation of densities between categories, especially for the short vowels, even though there is substantial category overlap. We had the choice to normalize duration by C-CuRE for all other accounts as well, but since the other approaches does not normalize other cues than formants, we believed it to be more informative to show how C-CuRE performs in comparison to other approaches that does not account for duration.

Figure \@ref(fig:separability-by-cue) clearly indicates that the category separability is affected along the cues in different ways. Some of these results are expected: for instance, it is expected that F0 will not support category separability much, since it is known to differ between talkers, but not between categories. For duration, it is expected that separability will improve for C-CuRE, as all other accounts do not transform the input but rather use the raw data (in ms). Given that categories emerge more clearly in F1-F2 space, it is also expected that separability will be higher when assessed along these cues. More surprising, however, is the result for F3, that clearly affects the normalization accounts' performance unexpectedly. These differences between the different cue indices, will undoubtly transfer to calculations of the separability index under the different assumptions about the relevant cues.
...
```{r separability-by-cue, fig.width= base.width * 2.5, fig.height = base.height * 6, out.height='100%', fig.align='center', fig.cap="(ref:separability-by-cue)"}
cowplot::plot_grid(
  cowplot::plot_grid(p.separability.F1_by_quantity + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()), 
                   ...
                   p.separability.F0_by_quantity + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()), ncol = 1, align = "hv", labels = c("A", "B", "C", "D")),
  p.separability.Duration_by_quantity, nrow = 2, align = "hv", rel_heights = c(4/5, 1.65/5), rel_widths = c(4/5,1/5), labels = c("", "E"))
```
...
```{r F3-densities, fig.width= base.width * 4, fig.height = base.height * 3, fig.align='center', fig.cap="(ref:F3-densities)"}
# Plot the F3 densities for each test fold , , , "[ɛ]", "[ɛː]", "[æː]", "[æ]", "[øː]", "[ø]", "[œː]", "[œ]", "[ɑː]", "[a]", ""[ɔ]", , "[ʊ]"
...
p.F3.test.short <- p.F3.test.long %+%
  (d.SwehVd.forStudy.long %>% 
  filter(Quantity == "short", category %in% c("[ɪ]", "[ʏ]", "[ɵ]", "[ɔ]", "[ʊ]"), fold_type == "test", Normalization.Type %in% c("no normalization (Hz)", "SyrdalGopal (Bark)", "Lobanov (Hz)")))
...
```{r fig.width= base.width * 4, fig.height = base.height * 3, out.width='100%', fig.align='center'}
d.SwehVd.forStudy.long %>% 
  filter(Quantity == "long", fold_type == "test", Normalization.Type %in% c("no normalization (Hz)", "SyrdalGopal (Bark)", "Lobanov (Hz)")) %>%
  ggplot(
...
(ref:duration-densities) Category duration densities for the Central Swedish long (**Panel A**) and short (**Panel B**) vowels in unnormalized Hz and C-CuRE normalized test data. Number in label indicates test-fold (1-5).
...
p.duration.test.long <- d.SwehVd.forStudy.long %>% 
  filter(Quantity == "long", fold_type == "test", Normalization.Type %in% c("no normalization (Hz)", "C-CuRE (Hz)")) %>%
  ggplot(
    aes(Duration)) +
  geom_density(
    mapping = aes(fill = category, linetype = Quantity), 
...
p.duration.test.short <- p.duration.test.long %+%
  (d.SwehVd.forStudy.long %>% 
  filter(Quantity == "short", fold_type == "test", Normalization.Type %in% c("no normalization (Hz)", "C-CuRE (Hz)")))

cowplot::plot_grid(p.duration.test.long, p.duration.test.short, labels = c("A", "B"), ncol = 1)
```
\end{landscape}

Figure \@ref(fig:separability) shows the separability index for the normalization accounts investigated, under the different assumptions about the relevant cues (Panel A-C) and depending on whether separability was calculated separately for the long and short vowels, or together. The first evaluation follows most previous research in assessing category separability along F1 and F2 (**Panel A**). We see that accounts that center against the talker's overall formant mean, such as C-CuRE [@mcmurray-jongman2011] or @nearey1978 (in blue), are among the highest performing normalization accounts, independent of vowel space size. Also, no matter the assumed perceptual scale, centering always improves category separability (compare transformation accounts, in grey, normalized with C-CuRE, in blue). Scaling accounts such as @lobanov1971 (in pink), also perform well at separating categories irrespective of vowel space size. Scale transformations (in grey), and the intrinsic accounts (in yellow), do not improve category separability compared to unnormalized Hz, at least not when assessed on the long vowels or the entire vowel space. These results overall replicate previous findings that have assessed category separability along F1 and F2 [e.g., @disner1980; @fabricius2009; @Flynn2011; @labov2010]. We also note that category separability is higher for the short vowels than the long vowels or the entire space. Given that the short vowels form a more centralized and dense space, this result might appear puzzling. However, as indicated in Figure \@ref(fig:swe-vowels), the long vowels are more overlapping than the short, especially in F1-F2 space. Overall, the separability index more or less aligns with the results from the visual inspection of scatter reduction introduced by normalization accounts (see Figures \@ref(fig:swe-vowels-normalized-long) and \@ref(fig:swe-vowels-normalized-short)). For instance, short vowels are visually more separated under C-CuRE comparing to the long vowels.

When including F3, and further F0 and vowel duration (Figure \@ref(fig:separability), **Panel B** and **Panel C**), we see changes in the normalization models' effect on category separability. Overall, the category separability is now lower. The centering accounts overall pattern the same way, scale transformations is now increasing separability relative to unnormalized models. The most drastic change in performance concerns the intrinsic and the scaling accounts, where the intrinsic accounts almost perform as good as when evaluated on F1 and F2, whereas the scaling accounts drop considerably in performance and now perform a lot worse than unnormalized models. This mirrors what was found when assessing the category separability separately for each cue (Figure \@ref(fig:separability-by-cue)). The high performance of Syrdal & Gopal and @miller1989c, and the low performance of @gerstman1968 and @lobanov1971, seems to be driven by their ability to increase category separability in F3. For a language like Swedish, where F3 carry information to vowel identity, intrinsic accounts such as SyrdalGopal and @miller1989c, that appear to do very well at separating categories along F3 and successfully distinguish catgories, seem to be particularly helpful, in comparison to other accounts. 

The separability index gives us a single number for the entire vowel system, which is convenient for comparability, but it does not capture everything. The only thing the measure cares about is the within-category over between-category variability. In our evaluation, this can become problematic for two reasons. First, in the second and third evaluations, we add dimensions to the separability index that are known to vary not as much by category as by talker (e.g., F0). Thus potentially providing the index with, in this context, irrelevant information, that can skew the index by adding within-category variability while leaving between-category variability (almost) unaffected. <!--REF Kuronen - for F3 across vowels.--> Second, given that the separability index is based on the sum of *squared variances*, just a few tokens located far away from the mean (be it because of measurement error or talker-specific factors) can affect the index disproportionally. This is clearly illustrated in the density plots, where many of the categories have a peak in one place, but a couple of points far away from the mean, resulting in overlapping densities. Whenever that is the case, the separability index will be affected/punish for that, however, it is not clear that those same points will have consequences for perception. That is, the index does not directly capture whether the direction in which these extreme points deviates from the mean, is expected to cause problem for perception or not. Next, we will in a coherent way assess this for perception.
