...
d.data <- d.SwehVd %>%
  ...
  filter(category == "[oː]", F1 > 380, F2 < 950) %>%
  ungroup() %>%
  add_row(Talker = "SW_000", category = "[oː]", F1 = c(550, 540, 543, 542, 530, 532, 520, 515, 540, 550, 530, 510, 505, 515, 520, 380, 385, 375, 388, 390), F2 = c(950, 900, 850, 800, 950, 850, 880, 870, 900, 910, 920, 850, 850, 850, 840, 680, 670, 675, 685, 660))

p.marginal.joint.1cat <- d.data %>%
  ggplot(
    aes(x=F2,
        y=F1,
        color = category)) +
  ...
  scale_x_reverse(breaks = scales::pretty_breaks(n = 4), limits = c(1200, 350)) +
  scale_y_reverse(breaks = scales::pretty_breaks(n = 4), limits = c(750, 300)) +
  geom_point(
    aes(x=quantile(d.data$F2, probs = .95),
        y=quantile(d.data$F1, probs = .05)),
    color = "black",
    size = 1) +
    guides(color = "none") +
  geom_point(
    aes(x=quantile(d.data$F2, probs = .95),
        y=quantile(d.data$F1, probs = .95)),
    color = "black",
    size = 1) +
    guides(color = "none") +
  geom_rug(
    mapping = aes(
      x=quantile(d.data$F2, probs = .95),
        y=quantile(d.data$F1, probs = .05)),
    color = "black",
    sides = "tr",
