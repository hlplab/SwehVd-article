...
# We thank audiences at the Department of Swedish language and multilingualism at Stockholm University and the ExLing 2022 conference in Paris for feedback on earlier presentations of this work. We are particularly grateful to Santiago Barreda for insightful discussion of issues in the evaluation of normalization accounts, Tomas Riad for discussion of Swedish vowel systems, Maryann Tan for collaboration in preparation of the SwehVd database, and to Nathan Young for feedback on segmentation procedures, as well as for access to, and support for, the [SweFA software](https://github.com/mcgarrah/LG-FAVE) for the automatic annotation of Swedish vowels. Finally, we thank Maryann Tan, Xin Xie XXXXXXX and other members of the Human Language Processing lab for feedback on earlier versions of the manuscript.
...
INCLUDE_SI = F     # switch on/off to include SI.
...
d.data.A <- m.io.SwehVd.F1F2 %>%
  filter(IO.Normalization.Type == "r_Hz", IO.crossvalidation_group == 1) %>%
  select(io) %>%
  unnest() %>%
  ...
  mutate_at(
      vars(contains("Sigma")),
      funs(map(. , .f = function(x)  {x * 1.2}))) %>%
  group_by(category) %>%
  mutate(
    samples = pmap(list(200, mu, Sigma), rmvnorm)) %>%
  unnest(samples) %>%
  mutate(
    F1 = samples[,1],
    F2 = samples[,2],
...
p.marginal.joint.1cat <- d.data.A %>%
  filter(category == "[ɑː]") %>%
  ggplot(
    aes(x=F2,
        y=F1,
  ...
  scale_y_reverse(breaks = scales::pretty_breaks(n = 4)) +
  geom_point(
    aes(x=quantile(d.data.A[d.data.A$category == "[ɑː]",]$F2, probs = .95),
        y=quantile(d.data.A[d.data.A$category == "[ɑː]",]$F1, probs = .05)),
    color = "black",
    size = 1) +
    guides(color = "none") +
  geom_point(
    aes(x=quantile(d.data.A[d.data.A$category == "[ɑː]",]$F2, probs = .95),
        y=quantile(d.data.A[d.data.A$category == "[ɑː]",]$F1, probs = .95)),
    color = "black",
    size = 1) +
    guides(color = "none") +
  geom_rug(
    mapping = aes(
      x=quantile(d.data.A[d.data.A$category == "[ɑː]",]$F2, probs = .95),
        y=quantile(d.data.A[d.data.A$category == "[ɑː]",]$F1, probs = .05)),
    color = "black",
    sides = "tr",
...
# SHOULD MODEL BE RUN UNDER THE ASSUMPTION OF ONE CATEGORY ONLY?????????
m <- lm(F1 ~ F2, data = d.data.A) %>% coefficients()

p.marginal.joint.2cat <- d.data.A %>%
  ggplot(
    aes(x=F2,
        y=F1,
        color = category)) +
  ...
  geom_text(
    aes(x=950,
        y=m[[1]]+m[[2]]*950),
    color = "black",
    label = dmvnorm(x = c(950,m[[1]]+m[[2]]*950), mean = d.data.A$mu[[1]], sigma = d.data.A$Sigma[[1]]) * 100,
    size = 3,
    nudge_y = 10) +
  geom_text(
    aes(x=1200,
        y=m[[1]]+m[[2]]*1200),
    color = "black",
    label = dmvnorm(x = c(1200,m[[1]]+m[[2]]*1200), mean = d.data.A$mu[[1]], sigma = d.data.A$Sigma[[1]]) * 100,
    size = 3,
    nudge_y = 10)

# Marginal densities along x axis
xdens.A <- cowplot::axis_canvas(p.marginal.joint.1cat, axis = "x") +
  #stat_function(fun = dnorm(5, mean = d.data.A$mu[[1]][1], sd = d.data.A$Sigma[[1]][1]), n = n) +
  geom_density(data = d.data.A %>% filter(category == "[ɑː]"), aes(x = F2, fill = category),
              alpha = 0.7, size = 0.2) +
  scale_fill_manual(values = colors.vowel.swe) +
  scale_x_reverse()
# Marginal densities along y axis
# Need to set coord_flip = TRUE, if you plan to use coord_flip()
ydens.A <- cowplot::axis_canvas(p.marginal.joint.1cat, axis = "y", coord_flip = TRUE) +
  geom_density(data = d.data.A %>% filter(category == "[ɑː]"), aes(x = F1, fill = category),
               alpha = 0.7, size = 0.2) +
  coord_flip() + 
  scale_fill_manual(values = colors.vowel.swe) +
  scale_x_reverse()
p.A <- cowplot::insert_xaxis_grob(p.marginal.joint.1cat, xdens.A, grid::unit(.2, "null"), position = "top")
p.A <- cowplot::insert_yaxis_grob(p.A, ydens.A, grid::unit(.2, "null"), position = "right")
p1 <- cowplot::ggdraw(p.A)

# Marginal densities along x axis
xdens.B <- cowplot::axis_canvas(p.marginal.joint.2cat, axis = "x") +
  #stat_function(fun = dnorm(5, mean = d.data.A$mu[[1]][1], sd = d.data.A$Sigma[[1]][1]), n = n) +
  geom_density(data = d.data.A, aes(x = F2, fill = category),
              alpha = 0.7, size = 0.2) +
  scale_fill_manual(values = colors.vowel.swe) +
  scale_x_reverse()
# Marginal densities along y axis
# Need to set coord_flip = TRUE, if you plan to use coord_flip()
ydens.B <- cowplot::axis_canvas(p.marginal.joint.1cat, axis = "y", coord_flip = TRUE) +
  geom_density(data = d.data.A, aes(x = F1, fill = category),
               alpha = 0.7, size = 0.2) +
  coord_flip() + 
  scale_fill_manual(values = colors.vowel.swe) +
  scale_x_reverse()
p.B <- cowplot::insert_xaxis_grob(p.marginal.joint.2cat, xdens.B, grid::unit(.2, "null"), position = "top")
p.B <- cowplot::insert_yaxis_grob(p.B, ydens.B, grid::unit(.2, "null"), position = "right")
p2 <-cowplot::ggdraw(p.B)
...
```{r gaussians, fig.width= base.width * 3, fig.height = base.height * 1+.5, out.height='80%', fig.align='center', fig.cap="(ref:gaussians)"}
