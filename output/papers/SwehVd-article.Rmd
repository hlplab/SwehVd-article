...
# To Do
  ...
  * Remove debugging code on SI when done.
  * Fill in XX.X% in general discussion.
  * Add text on difference matrix in SI.
  * Add REFS
    * double-check author names in bib-file
    * missing refs
  * FIGs
...
d.SwehVd.forStudy.long.within_category_MS <-
  ...
  summarise(
    ...
    #FOR DEBUGGING - AGAIN-------------------------------------------------------------------------------------------------------
    within_df_F0F2 = pmin(within_df_F0, within_df_F1, within_df_F2),
    within_df_allNOF0 = pmin(within_df_F1, within_df_F2, within_df_F3, within_df_Duration),
    within_df_allNOF3 = pmin(within_df_F0, within_df_F1, within_df_F2, within_df_Duration),
    ...
    #FOR DEBUGGING - AGAIN-------------------------------------------------------------------------------------------------------
    within_SS_F0F2 = within_SS_F0 + within_SS_F1 + within_SS_F2,
    within_SS_allNOF0 = within_SS_F1 + within_SS_F2 + within_SS_F3 + within_SS_Duration,
    within_SS_allNOF3 = within_SS_F0 + within_SS_F1 + within_SS_F2 + within_SS_Duration,
    # Get within-category MS
    across(
      .cols = paste0("within_SS_", c(cues, "F1F2", "F1F3", "F0F3", "F0F2", "all", "allNOF0", "allNOF3")),
...
get_SI <-
  ...
  left_join(
    d.SwehVd.forStudy.long %>%
      ...
      summarise(
        ...
        #DEBUGGING AGAIN----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        between_df_category_mean_F0F2 = pmin(between_df_category_mean_F0, between_df_category_mean_F1, between_df_category_mean_F2),
        between_df_category_mean_allNOF0 = pmin(between_df_category_mean_F1, between_df_category_mean_F2, between_df_category_mean_F3, between_df_category_mean_Duration),
        between_df_category_mean_allNOF3 = pmin(between_df_category_mean_F0, between_df_category_mean_F1, between_df_category_mean_F2, between_df_category_mean_Duration),
        ...
        #DEBUGGING AGAIN----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        between_SS_category_mean_F0F2 = between_SS_category_mean_F0 + between_SS_category_mean_F1 + between_SS_category_mean_F2,
        between_SS_category_mean_allNOF0 = between_SS_category_mean_F1 + between_SS_category_mean_F2 + between_SS_category_mean_F3 + between_SS_category_mean_Duration,
        between_SS_category_mean_allNOF3 = between_SS_category_mean_F0 + between_SS_category_mean_F1 + between_SS_category_mean_F2 + between_SS_category_mean_Duration,
        # Get between-category MS
        across(
          .cols = paste0("between_SS_category_mean_", c(cues, "F1F2", "F1F3", "F0F3", "F0F2", "all", "allNOF0", "allNOF3")),
  ...
  mutate(
    across(
      .cols = paste0("between_MS_", c(cues, "F1F2", "F1F3", "F0F3", "F0F2", "all", "allNOF0", "allNOF3")),
...
d.SwehVd.forStudy.SI.long <-
  ...
  select(c(Normalization.Type, crossvalidation_group, Quantity, SI_F1F2, SI_F1F3, SI_F0F2, SI_F0F3, SI_Duration, SI_F0, SI_F1, SI_F2, SI_F3, SI_all, SI_allNOF0, SI_allNOF3)) %>%
  pivot_longer(
...
\midrule
 & Barreda, 2021 & 30 female/male talkers of California English (15 tokens * 11 vowels) & Nearey2, Watt \& Fabricius, Lobanov & regression & against perceived category & Nearey2 (1), Watt \& Fabricius (2)\\
\cmidrule{2-7}
...
\cmidrule{2-5}
\cmidrule{7-7}
 & Nearey, 1989 & synthesized stimuli of male child/adult talker (based on male talker data from Fant, 1973, and Peterson \& Barney, 1952) & intrinsic (effects of F0, higher formants), extrinsic (effects of F1-F2 ensemble) & response patterns (F-ratio) & \multirow{-4}{2cm}{\raggedright\arraybackslash against perceived category} & effects of extrinsic ensemble (1), effects of F0 (2)\\
\cmidrule{2-7}
 & Richter, Feldman, Salgado \& Jansen, 2017 & Clopper \& Pisoni's (2006) NSP vowel corpus, 60 female/male talkers, 6 varieties (5 tokens * 10 vowels) & Vocal Tract Length Normalization (VTLN), Lobanov & discrimination model likelihoods & against perceived category (Feldman et al., 2009) & VTLN (1), Lobanov (2)\\
\cmidrule{2-7}
...
Paralleling Study 1, we trained and tested separate ideal observers for each combination of normalization account, cues, and training-test fold. Specifically, we use the exact same cross-validation folds as in Study 1. Each IO was trained on the training portion of the folded unnormalized and normalized data, and subsequently evaluated on the held-out test fold. This means that the parameters of each normalization account (e.g., the cue means in C-CuRE) and the resulting category parameters (the $\mu_c$s and $\Sigma_c$s for all categories) were set on the training data, and not changed for the test data. This reflects the realities of speech perception: although this is often ignored in evaluations of normalization accounts [e.g., @barreda2021; @mcmurray-jongman2011], listeners do not *a priori* know the cue means, cue variance, etc. of an unfamiliar talker. Rather, listeners need to incrementally *infer* those statistical properties from the talker's speech input [for discussion and a model, see @xie2022]. An additional advantage of cross-validation is that it gives us an estimate of the uncertainty about the model predictions. The performance of each IO during test is assessed by calculating the IO's predicted posterior probability of the *intended* category for each test token. This is identical to assuming Luce's choice rule for categorization [@luce1959], as in most models of speech perception and spoken word recognition.

### Advantages compared to separability index and similar measures of category variability
By using a categorization model to estimate the consequence of normalization for perception, we avoid the pitfalls of the separability index discussed in Study 1. First, by using the density of an acoustic input under the multivariate distribution of all cues, the ideal observers we employ capture the *joint* effect of all cues. This captures that an input can be an improbable instance of a category based on one of its cue values but a probable instance given the values for the other cues. In particular, since we assume multivariate Gaussian likelihoods, the model can account for category-specific covariances between cues. In the presence of strong correlations between cues, an acoustic input can be an improbable instance of a category based on the marginal distribution of each cue---i.e., if each cue is considered separately---but be a highly probable instance of that category if considered relative to the joint distribution of all cues (and vice versa). This is illustrated by Figure \@ref(fig:gaussians).<!-- TO DO: consider giving cumulative probability for both marginals and for joint.-->
...
(ref:gaussians) Using a perceptual model to evaluate normalization accounts avoids the pitfalls of separability/variability indices. **Panel A:** an acoustic token can be an improbable instance of a category if each cue is considered separately (the marginal densities along the sides of the plot), but highly probable if considered relative to the joint distribution of cues. Ellipses show bivariate Gaussian 95% cumulative probability mass. Coloured ticks on axis show cue mean. **Panel B:** two acoustic tokens that are equally far from the mean of one category can have radically different consequences for perception, depending on where the tokens fall relative to other categories.
...
Previous work found that the types of normalization accounts that performed well on other languages did not seem to perform well on Swedish vowel data [@disner1980]. However, as pointed out by Disner, the Swedish data differed from the data for other languages in that study, and the majority of studies on other languages. Here, we followed the majority of previous work on vowel productions and analyzed productions of hVd recordings. We find---both in Study 2 and in Study 1 (as long as F3 is not included)---that the same accounts found in previous work to perform well on other languages also perform well for the dense vowel space of Central Swedish. Specifically, Lobanov and centering approaches---incl. Nearey normalization and C-CuRE normalization---were the top-performing accounts, replicating the pattern found in previous studies on other languages [e.g., @adank2004; @carpenter1993; @escudero2007; @syrda1985]. 
...
(ref:separability-by-cue-SG) Separability indices of the two versions of the Syrdal & Gopal (1986) account for long vowels, short vowels, and long and short vowels together (columns), shown for four of the five cues considered in this study (rows), and the combined F1-F2. Labels indicate mean across the five test folds. Intervals show average bootstrapped 95% confidence intervals across the test folds. Note that the ranges of the y-axes varies across plots.
...
```{r separability-by-cue-SG, fig.width=base.width * 4, fig.height=base.height * 5.5, fig.align='center', fig.cap="(ref:separability-by-cue-SG)"}
...
p.accuracy.io.overall.SG <- plot_io_results(m.io.SwehVd.SG) + facet_grid(Cue_space ~ factor(IO.Quantity, levels = c("long", "short", "all"), labels = c("long vowels", "short vowels", "all vowels"))) +
  ...
  theme(
    axis.text.x = ggtext::element_markdown(angle = 60, vjust = 1, hjust=1, colour = "#E6BE8A"))
```
(ref:accuracy-SG) Predicted recognition accuracy of ideal observer under two versions of the Syrdal & Gopal (1986) account for long vowels, short vowels, and long and short vowels together (columns), shown for the F1-F2 cue combination. Labels indicate mean across the five test folds. Intervals show average bootstrapped 95% confidence intervals across the test folds. The dashed horizontal line indicates chance (different across columns because of the different number of long and short vowels).

```{r accuracy-SG, fig.width=base.width * 3, fig.height=base.height * 1.5, fig.align='center', fig.cap="(ref:accuracy-SG)"}
p.accuracy.io.overall.SG
```

We also evaluated the two Syrdal & Gopal implementations in terms of model predictions for perception. Figure \@ref(fig:accuracy-SG) displays the categorization accuracy for models trained on normalized data under the two implementations of the Syrdal & Gopal account. Replicating the results from the separability index, the first implementation using F2-F1 for the second dimension, outperforms the implementation using F3-F2 bark-difference measure. These results taken together indicate that the F2-F1 implementation is more suitable for the materials used here, we therefore decided to use the first implementation throughout this paper.
...
cowplot::plot_grid(legend.confusion,
  cowplot::plot_grid(p.confusion.all.F1F2, p.confusion.all.F1F3, p.confusion.all.all + theme(legend.position = "none"), align = "hv", axis = "btlr", ncol = 1),
    nrow = 2, rel_heights = c(.08,1)) 
```
To shed further light on vowel-specific differences in model predictions, we plot difference matrices. Figure \@ref(fig:diff-matrix) visualizes the differences in predictions between the worst and best performing models under the different assumptions about the relevant cues and vowel spaces. <!-- AP: add text here-->
...
p.diff.long.F1F2 <- p.confusion.long.F1F2 %+%
  (m.io.SwehVd %>%
    ...
    aes(fill = Difference) +
    scale_fill_gradient2("Difference in\nposterior probability", limits = c(-.25,.25), breaks = seq(-.25, .25, .05)) +
...
(ref:diff-matrix) Difference matrices illustrating the differences in predictions between the best and the worst performing models, under different assumptions about the relevant cues (F1-F2 **top**, F1-F3 **middle**, and F0-F3, and duration **bottom**) and vowel space sizes (long vowels **column A**, short vowels **column B**, all vowels **column C**). Vowel intended by talker (x-axis) is plotted against vowel selected by IO model (y-axis). Color fill indicates the difference in the posterior probability of the models predicting the intended vowel. More **purple** indicates an increase in posterior probability for the former over the latter model, more **red** indicates an advantage for the latter over the former.

\begin{landscape}

```{r diff-matrix, fig.width=base.width * 3.8, fig.height=base.height * 3.25, fig.align='center', fig.cap="(ref:diff-matrix)"}
cowplot::plot_grid(legend.diff,
  cowplot::plot_grid(
    cowplot::plot_grid(p.diff.long.F1F2, p.diff.long.F1F3, p.diff.long.all, align = "hv", axis = "btlr", ncol = 1),
    cowplot::plot_grid(p.diff.short.F1F2, p.diff.short.F1F3, p.diff.short.all, align = "hv", axis = "btlr", ncol = 1),
    cowplot::plot_grid(p.diff.all.F1F2, p.diff.all.F1F3, p.diff.all.all + theme(legend.position = "none"), align = "hv", axis = "btlr", ncol = 1),
    ncol = 3, labels = c("A", "B", "C")),
    ncol = 2, rel_widths = c(.08,1))
```
\end{landscape}
