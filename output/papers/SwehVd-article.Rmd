...
All data and code for this article can be downloaded from OSF at <https://osf.io/zb8gx/>. This article is written in R markdown, allowing readers to replicate our analyses using freely available software [@R; @RStudio], while changing any of the parameters of our models. Readers can revisit and alter the assumptions we make---for example, uniform priors, models of linguistic representations, the normalization accounts selected. The supplementary information (SI) lists the software/libraries required to compile this document.

# Methods and materials {#sec:methodsII -}
In this section, we begin by introducing the new phonetically annotated corpus of Central Swedish vowel productions employed in the present study. We then present the perceptual model that we use for assessing the predicted effects of different normalization accounts.
...
In this study, we have included a temporal cue alongside the commonly used spectral cues when modelling expected effects of normalization for vowel perception. <!--AP: moved from introduction, as suggested by R2.-->To our knowledge, duration has not been considered in previous studies on normalization, presumably because the present study is the first to evaluate normalization accounts against a vowel system with a systematic quantity distinction (long vs. short vowels). <!--AP:end of move.-->Overall, including duration improves the model accuracy across evaluations. More specifically, when all vowels are considered and duration is included as cue, we see the largest increase in model performance across models, with the best-performing accounts moving from `r d.max.acc.toINTEND %>% filter(Cue_space == "F1-F2", IO.Quantity == "all", IO.Normalization.Type == "CCuRE_Hz") %>% pull(mean) %>% round(digits = 3) * 100`% recognition accuracy to `r d.max.acc.toINTEND.diff %>% filter(Cue_space == "F0-F3, duration", IO.Quantity == "all") %>% pull(mean.x) %>% round(digits = 3) * 100`% when chance is as low as 4.8%. This demonstrates the importance of duration as acoustic-perceptual cue for vowel quantity distinctions. It furthermore suggests that temporal cues, such as duration, are susceptible to normalization, and that vowel normalization mechanisms operate not only in frequency domains but also time domains. General purpose accounts that can take any type of cue as input, such as C-CuRE, would presumably have an advantage against vowel specific accounts, even more so in languages with a systematic quantity distinction, such as Swedish. Future studies could investigate the relative advantage of general purpose accounts for languages that does not have a systematic quantity distinction, to see whether the results generalize.
