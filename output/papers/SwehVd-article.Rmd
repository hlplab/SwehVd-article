...
# To Do
  ...
  * REFs
    * add REF on coarticulation with /d/ in coda
...
Figure \@ref(fig:diphthongs) shows the trajectory of the average F1-F2 values across the five time-points for each of the vowels in the SwehVd database. It is clear from this plot that there is some movement across time-points. Across vowels, the movement is largest going from time-point 3 to 5, that is, from vowel mid-point to at 80% of the vowel. The vowels that exhibit most movement are [`r linguisticsdown::cond_cmpl("eː")`] and  [`r linguisticsdown::cond_cmpl("oː")`] [in line with e.g., @eklund1997; @elert2000]. The high front vowels ([`r linguisticsdown::cond_cmpl("iː")`], [`r linguisticsdown::cond_cmpl("yː")`], and their short counterparts) display very little movement. The directionality is primarily along F1, towards a higher position for all vowels besides [`r linguisticsdown::cond_cmpl("eː")`] and  [`r linguisticsdown::cond_cmpl("oː")`], that move towards a more centralized position. There is also some movement along F2. The directionality and systematicity of the movement was confirmed when running separate mixed effect regressions for each vowel, with F1 as dependent variable and time-point as predictor, and the interaction of time-point and Talker as random effects.<!-- AP: So, where the t-value was always negative and significant for all time-points 3-5? For F2: positive movement - ʊ, ɔ, œ, ɵ, œː ; negative - ø, ɛ, øː, ɛː, eː, ʉː, yː --> The models indicate a significant and systematic upward movement along F1 from mid-point to 80% into the vowel for [`r linguisticsdown::cond_cmpl("uː")`], [`r linguisticsdown::cond_cmpl("ʉː")`], [`r linguisticsdown::cond_cmpl("æː")`], [`r linguisticsdown::cond_cmpl("ɛ")`], [`r linguisticsdown::cond_cmpl("æ")`], [`r linguisticsdown::cond_cmpl("ø")`], [`r linguisticsdown::cond_cmpl("œ")`], [`r linguisticsdown::cond_cmpl("a")`], and [`r linguisticsdown::cond_cmpl("ɔ")`], and a downward movement for [`r linguisticsdown::cond_cmpl("eː")`] and  [`r linguisticsdown::cond_cmpl("oː")`]. These results does not entirely replicate previous descriptions of diphthongisation of Swedish vowels. In @eklund1997, all long vowels moved towards a more open quality. They however used the letter names for recording, which might indicate that what we see here is partly the result of co-articulation with the subsequent /d/ [@REFS].<!-- AP: check and cite papers here. e.g. Sussman et al., 1997, Parker, 1974; what do you expect from a final /d/? Also, check what context Elert is using-->

(ref:diphthongs) The trajectory of all vowels across the five time points, along F1-F2. Each number correspond to the five time-points at 20%, 35%, 50%, 65% and 80% into the vowel.
...
```{r swe-vowels, out.width = '100%', out.height = '100%', fig.align='center', fig.cap="(ref:swe-vowels)"}
d.SwehVd %>% 
  filter(Transcribed_vowel == "targeted", Word != "hädd") %>%
  # Get the geometric mean F1-F2 across the five time points
...
d.SwehVd.forStudy.SI.long <- 
  ...
  # DEBUGGING --------------------------include all separate SIs for now
...
# Plot the F3 densities for each test fold
p.F3.test.long <- d.SwehVd.forStudy.long %>% 
  filter(Quantity == "long", fold_type == "test", Normalization.Type %in% c("no normalization (Hz)", "SyrdalGopal (Bark)", "Lobanov (Hz)")) %>%
  ggplot(
    aes(F3)) +
  geom_density(
    mapping = aes(fill = category, linetype = Quantity), 
    alpha = .3, position = "identity") +
  scale_colour_manual(name = "category", values = colors.vowel.swe, aesthetics = c("colour", "fill")) +
  guides(linetype = "none", shape = guide_legend(override.aes = list(alpha = .4))) +
  facet_wrap(Normalization.Type ~ crossvalidation_group, scales = "free", ncol = 5)

p.F3.test.short <- p.F3.test.long %+%
  (d.SwehVd.forStudy.long %>% 
  filter(Quantity == "short", fold_type == "test", Normalization.Type %in% c("no normalization (Hz)", "SyrdalGopal (Bark)", "Lobanov (Hz)")))
```
...
To further investigate the effects of neighbouring categories, and understand which categories are more easily confused by the IOs, and with what, we plot a confusion matrix that displays the best and worst performing IOs' predictions about the entire Swedish vowel space (see Figure \@ref(fig:confusion-matrix)). Three general observations can be made. (1) When there is high competition from many close neighbours, models are more inclined to confuse categories with one of their neighbours. This is especially evident for the overlapping high vowels, but also for the allophones to [`r linguisticsdown::cond_cmpl("ɛ")`] and [`r linguisticsdown::cond_cmpl("ø")`]. (2) Given that the closest neighbour is not necessarily the categories' short/long counterpart, it is not primarily the case that quantity distinctions are the cause of models' confusion. (3) As more information is given the models, confusability declines substantially (panel B, Figure \@ref(fig:confusion-matrix)). Including F3 as cue reduces the confusability among some of the high vowels, including duration reduces quantity confusions down to one category distinction (between [`r linguisticsdown::cond_cmpl("uː")`] and [`r linguisticsdown::cond_cmpl("ʊ")`], for the worst performing model). 
...
This paper has investigated the effect of low-level pre-linguistic normalization on Stockholm Swedish vowels, by assessing different accounts' effects on category separability, and on predicted consequences for perception. Evaluating 15 different normalization accounts against the dense vowel space of Swedish, has allowed us to assess whether the same normalization accounts that work well for vowel systems with fewer categories, work equally well for more complex systems. Investigating a language with quantity distinction has furthermore allowed for a bridging of the literature on vowel-specific accounts and on general normalization accounts [such as C-CuRE; @mcmurray-jongman2011], and for expanding on previous research that primarily has focused on the first two (or three) formants. 

The speech materials used was a subset of the SwehVd database, a new phonetically annotated database of Stockholm Swedish vowels that was presented and released in this paper. The following methods were used for evaluation. In Study I, we compared the reduction of within-category variability relative to between-category variability by calculating a separability index (i.e., an F-statistics). This methodology facilitates comparison to previous studies, that has most often assessed the efficacy of normalization accounts by means of increase in category overlap or category separability, or decrease in category variability [@disner1980; @fabricius2009; @Flynn2011; @hindle1978; @labov2010]. In Study II, we assessed the predicted consequences for perception by training ideal observer models on the acoustic data transformed into different normalization spaces, under different assumptions about the relevant cues. Previous studies have primarily used models of linear discriminant analysis [e.g., @adank2004; @syrda1985], fuzzy ARTMAP and K-nearest neighbour [e.g., @carpenter1993], or constraint ranking grammars [e.g., @escudero2007]. However, we find several advantages with the modelling approach adopted here. First, IO models have 0 degrees of freedom in the link from production to perception, thus removing the researchers' degrees of freedom in the evaluation of normalization accounts. Second, representing the cue likelihood for each category as multivariate Gaussian distributions [e.g., @kronrod2016; @norris-mcqueen2008; @xie2021cognition], strikes a middle ground between less parsimonious models such as exemplar models [e.g., @johnson1997; @pierrehumbert2001], and more parsimonious, such as models of cue integration over multiple separate univariate Gaussians [e.g., @toscano-mcmurray2010]. And finally, we find that the 5-fold cross-validation approach more realistically reflects how parameters used for normalization is incrementally inferred from the talker's speech input.^[We would like to remind readers that all of the assumptions in the model (e.g., uniform priors, models of linguistic representations) can be revisited and altered, as this paper is written in R markdown and all data and code is provided in the SI/OFS.]

The results from Study I showed that the same type of accounts that have been proven successful in previous evaluations on other languages, are also successful in improving category separability on Swedish vowels. The increase in category separability for the best performing normalization accounts, seems to be equally large as those reported by @disner1980 for the other languages in her cross-linguistic study.<!-- AP: go back to Disner here--> Furthermore, here, simple centering performs as good, or even better, for Swedish as it did for the other languages in her study. In conclusion, it might well be the case that the (somewhat) diverging results for Swedish in @disner1980's study, could be an effect of the nature of the dataset used <!-- AP: or male vs female--> However, ....<!-- AP: check results om Labov, Barreda/Nearey, that also used Fstats + this cannot be finalized until we fully understand what is going on with the sep index-->. 

Study II indicated that psycho-acoustic scale transformations alone does not make as strong predictions for perception as normalization accounts do. Lobanov and centering approaches, vowel-specific or general such as Nearey or C-CuRE, are the top-performing accounts, which generally replicates previous studies comparing predictions against production data [e.g., @adank2004; @barreda2018a; @carpenter1993; @escudero2007; @syrda1985]. As more cues are included, presumably making models more human-like, the relative advantage of Lobanov normalization seems to decrease, possible indicating that simple centering might make up a plausible candidate for a mechanism supporting robust inter-talker perception. More robust evidence for such a claim, would of course be to compare models against perceptual data. This is also our intention, moving forward... 

To summarise, comparing normalization accounts against Swedish has showed that normalization seem to work equally well for more crowded vowel systems. Evaluating accounts using more cues shows that general accounts that are not specific to phonological categories or phonetic cues, work as good as other accounts...
...
<!-- AP: B&N does this whole analysis of suitablility of approaches as models of human cognition, where these measures are included, but is it relevant here?: calculating amount of error in normalization parameter estimates [e.g., @barreda2018a]-->
